...
-- Replace the generate_recurring_events function with the improved version
...
BEGIN
  -- LOG
  RAISE NOTICE 'Starting recurring child creation: parent %, start %, end %, pattern %, until %',
    p_parent_event_id, p_start_date, p_end_date, p_repeat_pattern, p_repeat_until;

  LOOP
    -- Pick pattern interval
    ...
    -- Break loop if date is after repeat_until (midnight, so <= is not included)
    IF v_next_start::date > p_repeat_until THEN
      EXIT;
    END IF;

    -- LOG
    RAISE NOTICE 'Creating child event at %', v_next_start;

    -- Actually insert child event, copying from parent
    INSERT INTO events (
    ...
    SELECT
      title, user_surname, user_number, social_network_link, event_notes, event_name,
      v_next_start, v_next_end, payment_status, payment_amount,
...
-- Update the save_event_with_persons function to properly call the generator with better logging
...
BEGIN
  -- LOG input data
  RAISE NOTICE 'save_event_with_persons called with event: %', p_event_data;
  ...
  RAISE NOTICE 'save_event_with_persons: v_is_recurring=%, v_repeat_pattern=%, v_repeat_until=%', 
  ...
  IF p_event_id IS NULL THEN
    ...
    -- CRITICAL: Generate recurring instances using PERFORM (not SELECT)
    IF v_is_recurring = true AND 
      ...
      -- Use PERFORM to call the function and get the count
      ...
      RAISE NOTICE 'Called generate_recurring_events for event %', v_event_id;
    ELSE
