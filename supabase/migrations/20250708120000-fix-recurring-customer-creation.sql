...
-- Fix the save_event_with_persons function to only create customers for parent events, not recurring instances
...
DECLARE
  ...
  v_is_recurring_instance BOOLEAN := false;
BEGIN
  ...
  ELSE
    ...
    -- Check if this is a recurring instance (has parent_event_id)
    SELECT parent_event_id IS NOT NULL INTO v_is_recurring_instance
    ...
    -- Only delete and recreate customers if this is NOT a recurring instance
    -- Recurring instances should not have their own customer records
    IF NOT v_is_recurring_instance THEN
  ...
  -- CRITICAL FIX: Only insert customers for parent events, NOT for recurring instances
  -- This prevents creating duplicate customers for each event in a recurring series
  IF NOT v_is_recurring_instance AND (p_event_id IS NULL OR NOT EXISTS(
    SELECT 1 FROM events WHERE id = v_event_id AND parent_event_id IS NOT NULL
  )) THEN
    -- Insert additional persons only for parent events
    ...
    LOOP
      ...
      RAISE NOTICE '👤 Created customer for parent event: %', v_event_id;
    END LOOP;
  ELSE
    RAISE NOTICE '⚠️ Skipping customer creation for recurring instance: %', v_event_id;
  END IF;
