...
-- Create a function to check for time overlaps with existing events
CREATE OR REPLACE FUNCTION public.check_time_overlap(
  p_user_id uuid,
...
AS $function$
BEGIN
  -- Check for overlaps with regular events
  RETURN QUERY
  SELECT 
    e.id as event_id,
  ...
  -- If business_id is provided, also check approved booking requests
  IF p_business_id IS NOT NULL THEN
    RETURN QUERY
    SELECT 
      br.id as event_id,
      br.title as event_title,
      br.start_date as event_start,
      br.end_date as event_end,
      'booking_request'::text as event_type
    FROM booking_requests br
    WHERE br.business_id = p_business_id
      AND br.status = 'approved'
      AND br.deleted_at IS NULL
      AND (p_exclude_event_id IS NULL OR br.id != p_exclude_event_id)
      AND (
        -- Check for any time overlap
        (br.start_date < p_end_date AND br.end_date > p_start_date)
      );
  END IF;
...
-- Update the save_event_with_persons function to include time conflict validation
...
BEGIN
  ...
  -- Get business_id for the user to check booking conflicts
  ...
  FOR v_conflict_record IN 
    SELECT * FROM check_time_overlap(
      p_user_id, 
      v_start_date, 
      v_end_date, 
      p_event_id,
  ...
  LOOP
    IF v_conflict_message = '' THEN
      v_conflict_message := 'Time conflict detected with existing events: ';
    ELSE
      v_conflict_message := v_conflict_message || ', ';
    END IF;
    
    v_conflict_message := v_conflict_message || 
      '"' || v_conflict_record.event_title || '" (' || 
      to_char(v_conflict_record.event_start, 'YYYY-MM-DD HH24:MI') || ' - ' ||
      to_char(v_conflict_record.event_end, 'YYYY-MM-DD HH24:MI') || ')';
  END LOOP;
  ...
  IF p_event_id IS NULL THEN
    ...
    INSERT INTO events (
      ...
      user_id, type, is_recurring, repeat_pattern, repeat_until, created_at, updated_at
    ) VALUES (
      v_safe_title,
      v_safe_user_surname,
