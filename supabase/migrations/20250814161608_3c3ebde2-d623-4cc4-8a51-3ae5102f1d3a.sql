-- Fix the save_event_with_persons function to properly use sub-user metadata parameters
-- This will ensure that when sub-users create events and customers, they get proper ownership tracking
...
BEGIN
  ...
  RAISE NOTICE '🔍 Event data received: is_recurring=%, repeat_pattern=%, repeat_until=%, start_date=%, reminder_at=%, email_reminder_enabled=%, created_by=% (%)', 
               v_is_recurring, v_repeat_pattern, v_repeat_until, v_start_date::date, v_reminder_at, v_email_reminder_enabled, p_created_by_name, p_created_by_type;
  ...
  IF p_event_id IS NULL THEN
    -- Create new event with proper metadata
    INSERT INTO events (
      ...
      p_created_by_type,        -- USE THE PASSED PARAMETER, NOT HARDCODED 'admin'
      p_created_by_name,        -- USE THE PASSED PARAMETER
      p_last_edited_by_type,    -- USE THE PASSED PARAMETER
      p_last_edited_by_name,    -- USE THE PASSED PARAMETER
      NOW(),
      NOW()
  ...
  ELSE
    -- Update existing event with proper metadata
    UPDATE events SET
      title = v_safe_title,
      ...
      last_edited_by_type = p_last_edited_by_type,     -- USE THE PASSED PARAMETER
      last_edited_by_name = p_last_edited_by_name,     -- USE THE PASSED PARAMETER
      updated_at = NOW()
  ...
  LOOP
    INSERT INTO customers (
      ...
      v_start_date,
      v_end_date,
      p_created_by_type,        -- USE THE PASSED PARAMETER FOR CUSTOMERS TOO
      p_created_by_name,        -- USE THE PASSED PARAMETER FOR CUSTOMERS TOO
      p_last_edited_by_type,    -- USE THE PASSED PARAMETER FOR CUSTOMERS TOO
      p_last_edited_by_name     -- USE THE PASSED PARAMETER FOR CUSTOMERS TOO
    );
