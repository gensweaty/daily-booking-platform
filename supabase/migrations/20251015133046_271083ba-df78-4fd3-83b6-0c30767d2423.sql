-- Drop ALL versions of save_event_with_persons function to fix schema cache error
-- This ensures no old signatures remain before creating the correct 8-parameter version

-- Drop all overloaded versions of the function
DROP FUNCTION IF EXISTS public.save_event_with_persons(jsonb, jsonb, uuid, uuid) CASCADE;
DROP FUNCTION IF EXISTS public.save_event_with_persons(jsonb, jsonb, uuid, uuid, text, text, text, text) CASCADE;
DROP FUNCTION IF EXISTS public.save_event_with_persons(jsonb, jsonb, uuid, uuid, boolean, text, boolean, text, text, text) CASCADE;
DROP FUNCTION IF EXISTS public.save_event_with_persons CASCADE;

-- Now create the correct 8-parameter version
