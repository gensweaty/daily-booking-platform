...
-- 5) Keep start_public_board_dm callable with BOTH param name styles.
--    We'll provide a wrapper that accepts p_other_* and calls the main one.

-- Main implementation (p_board_owner_id, p_sender_email, p_target_type, p_target_id)
...
-- Wrapper with the old parameter names (so existing frontend calls keep working)
CREATE OR REPLACE FUNCTION public.start_public_board_dm(
  p_board_owner_id uuid,
  p_sender_email  text,
  p_other_type    text,
...
AS $$
BEGIN
  RETURN public.start_public_board_dm(
    p_board_owner_id => p_board_owner_id,
    p_sender_email   => p_sender_email,
    p_target_type    => p_other_type,
    p_target_id      => p_other_id
  );
END
$$;