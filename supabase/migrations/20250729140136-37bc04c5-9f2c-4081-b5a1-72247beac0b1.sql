...
-- First, let's restore the save_event_with_persons function to its original working state
-- Remove any references to check_time_overlap and restore original functionality
...
-- Remove the problematic check_time_overlap function if it exists
DROP FUNCTION IF EXISTS public.check_time_overlap(uuid, timestamp with time zone, timestamp with time zone, uuid, uuid);
