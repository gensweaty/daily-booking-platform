-- Fix recurring event customer data propagation
-- When updating additional persons for a recurring event, apply changes to ALL instances in the series
...
BEGIN
  ...
  -- 🔥 CRITICAL FIX: Handle customer updates for recurring events
  -- Check if this is a recurring event and collect all related event IDs
  ...
  IF p_event_id IS NOT NULL THEN
    ...
    SELECT 
      CASE 
        WHEN is_recurring = true THEN ARRAY[id] -- This is the parent event
        WHEN parent_event_id IS NOT NULL THEN ARRAY[parent_event_id] -- This is a child, use parent
        ELSE ARRAY[id] -- Regular single event
      END
    INTO v_recurring_event_ids
    FROM events 
    WHERE id = p_event_id AND user_id = p_user_id;
    
    -- If this is a recurring series, get ALL events in the series
  ...
  LOOP
    -- Insert the customer record for EACH event in the recurring series
    IF array_length(v_recurring_event_ids, 1) > 0 THEN
