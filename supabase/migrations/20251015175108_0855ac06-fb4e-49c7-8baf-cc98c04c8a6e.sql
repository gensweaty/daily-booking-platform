-- Harden edit_single_event_instance_v2 to prevent "this event" from touching the series
-- This ensures proper RLS bypass and handles the first occurrence explicitly

-- Make the function SECURITY DEFINER so it can create exclusions/children under public board context
DROP FUNCTION IF EXISTS public.edit_single_event_instance_v2(uuid,uuid,jsonb,jsonb,timestamptz,timestamptz,text,text,boolean);
...
DECLARE
  ...
  v_final_instance_start timestamptz;
  v_final_instance_end timestamptz;
BEGIN
  ...
  -- CRITICAL: Handle first occurrence (series root) - if instance times are missing, use the parent's original times
  IF p_instance_start IS NULL OR p_instance_end IS NULL THEN
    SELECT start_date, end_date INTO v_final_instance_start, v_final_instance_end
    FROM events
    WHERE id = v_parent_id AND user_id = p_user_id AND deleted_at IS NULL;
  ELSE
    v_final_instance_start := p_instance_start;
    v_final_instance_end := p_instance_end;
  END IF;

  RAISE NOTICE '[edit_single_event_instance_v2] Splitting instance: parent=%, original_window=%-%', 
               v_parent_id, v_final_instance_start, v_final_instance_end;
  ...
  INSERT INTO events (
    ...
    COALESCE((p_event_data->>'start_date')::timestamptz, v_final_instance_start),
    COALESCE((p_event_data->>'end_date')::timestamptz, v_final_instance_end),
  ...
  INSERT INTO events (
    ...
    v_final_instance_start,
    v_final_instance_end,
  ...
  RAISE NOTICE '[edit_single_event_instance_v2] Successfully split: new_event=%, excluded_window=%-%', 
               v_new_event_id, v_final_instance_start, v_final_instance_end;
...
GRANT EXECUTE ON FUNCTION public.edit_single_event_instance_v2(uuid,uuid,jsonb,jsonb,timestamptz,timestamptz,text,text,boolean) TO anon, authenticated;