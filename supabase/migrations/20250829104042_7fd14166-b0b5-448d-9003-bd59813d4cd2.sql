...
IMMUTABLE AS $$
  ...
  FROM (
    SELECT a_type, a_id, b_type, b_id,
           dm_sort_key(a_type, a_id) as a_key,
           dm_sort_key(b_type, b_id) as b_key
  ) s
  , LATERAL (
     SELECT CASE WHEN a_key <= b_key THEN a_type ELSE b_type END  as left_type,
            CASE WHEN a_key <= b_key THEN a_id   ELSE b_id   END  as left_id,
            CASE WHEN a_key <= b_key THEN b_type ELSE a_type END  as right_type,
            CASE WHEN a_key <= b_key THEN b_id   ELSE a_id   END  as right_id
  ) o;
$$;
