...
-- 4) Backfill dm_pair_key for existing DMs (fixed query)
...
FROM (
  SELECT 
    channel_id,
    ROW_NUMBER() OVER (PARTITION BY channel_id ORDER BY created_at) as rn,
    user_id,
    sub_user_id,
    user_type
  FROM chat_participants
) ranked
JOIN chat_participants p1 ON p1.channel_id = ranked.channel_id AND ranked.rn = 1
JOIN chat_participants p2 ON p2.channel_id = ranked.channel_id AND p2.id != p1.id
WHERE ch.id = ranked.channel_id 
  AND ch.is_dm = true 
