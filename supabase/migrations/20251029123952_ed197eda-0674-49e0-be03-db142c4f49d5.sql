-- Fix unread_counters to count AI messages as unread even when sender_user_id = viewer_id
-- This ensures visual badge notifications appear for AI chat messages and reminders

-- Ensure message_type column exists for future use
DO $$ 
BEGIN
  IF NOT EXISTS (
...
-- Update unread_counters function to count AI messages as unread
  ...
  msgs as (
    select m.channel_id, m.created_at,
     ...
     where m.owner_id = p_owner_id
       -- Don't count my own messages EXCEPT AI messages (they should always count as unread)
       and not (
         ((p_viewer_type = 'admin' and m.sender_user_id = p_viewer_id)
          or (p_viewer_type = 'sub_user' and m.sender_sub_user_id = p_viewer_id))
         and m.sender_name != 'Smartbookly AI'  -- Exception: AI messages always count
       )
  ),
  channel_cte as (
    select r.channel_id,
