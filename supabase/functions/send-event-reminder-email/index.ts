...
const getEventEmailContent = (language: string, eventTitle: string, eventStartTime: string, eventNotes?: string) => {
  let subject, body;
  ...
  if (language === 'ka') {
    ...
    body = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p style="font-size: 14px; color: #666;">
          <strong>დრო:</strong> ${eventStartTime}
        </p>
    ...
    body = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p style="font-size: 14px; color: #666;">
          <strong>Programado para:</strong> ${eventStartTime}
        </p>
    ...
    body = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p style="font-size: 14px; color: #666;">
          <strong>Scheduled for:</strong> ${eventStartTime}
        </p>
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (eventId) {
      ...
      const deduplicationKey = `${event.id}_${userEmail}`;
      ...
      // Format event start time using the new function with proper locale and timezone
      const formattedTime = formatReminderTimeForLocale(event.start_date, language);

      // Get localized email content
      const { subject, body: emailBody } = getEventEmailContent(
        language, 
        event.title || event.user_surname || 'Untitled Event', 
        formattedTime, 
        event.event_notes
      );
      ...
      console.log(`✅ Reminder email sent for event ${event.id} to ${userEmail} in language ${language}`);
