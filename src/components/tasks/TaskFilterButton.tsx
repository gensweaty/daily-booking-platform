import { Filter, Check, User, UserPlus } from "lucide-react";
...
import { useTaskFilters, SortOrder, FilterType } from "@/hooks/useTaskFilters";
...
export const TaskFilterButton = ({ boardOwnerId }: TaskFilterButtonProps) => {
  const { filters, setSortOrder, setFilterType, resetFilters } = useTaskFilters();
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button 
          variant="outline" 
          className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-4 text-xs sm:text-sm relative ${
      ...
      <DropdownMenuContent align="end" className="w-56 bg-background">
        ...
        <DropdownMenuItem
          ...
          className="flex items-center justify-between cursor-pointer"
        >
          <span>{isGeorgian ? <GeorgianAuthText>ყველა დავალება</GeorgianAuthText> : 'All Tasks'}</span>
        ...
        <DropdownMenuSub>
          <DropdownMenuSubTrigger className="cursor-pointer">
            ...
            <span>{isGeorgian ? <GeorgianAuthText>მინიჭებული</GeorgianAuthText> : 'Assigned To'}</span>
            ...
            {isLoading ? (
              ...
              assignees.map((assignee) => (
                <DropdownMenuItem
                  ...
                  onClick={() => setFilterType('assigned', assignee.id, assignee.type)}
