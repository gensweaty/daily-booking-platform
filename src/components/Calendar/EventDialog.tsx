...
export const EventDialog = ({
  ...
  const loadAdditionalPersons = async (targetEventId: string) => {
    try {
      ...
      const {
        data: customers,
        error
      } = await supabase.from('customers').select('*').eq('event_id', actualEventId).eq('type', 'customer').is('deleted_at', null);
      if (error) {
  ...
  const loadExistingFiles = async (targetEventId: string) => {
    try {
      ...
      const {
        data: eventFiles,
        error
      } = await supabase.from('event_files').select('*').eq('event_id', actualEventId);
      if (error) {
  ...
  const loadParentEventData = async (parentId: string) => {
    try {
      const {
        data: parentEvent,
        error
      } = await supabase.from('events').select('*').eq('id', parentId).single();
  ...
  const uploadFiles = async (eventId: string) => {
    ...
    const uploadPromises = files.map(async file => {
      ...
      const {
        error: uploadError
      } = await supabase.storage.from('event_attachments').upload(fileName, file);
      if (uploadError) {
    ...
    try {
      ...
      for (const recipient of recipients) {
        try {
          ...
          const emailResult = await sendEventCreationEmail(recipient.email, recipient.name, businessData.business_name || '', eventData.start_date, eventData.end_date, recipient.paymentStatus,
          // Use individual payment status
          recipient.paymentAmount,
          // Use individual payment amount
          businessData.contact_address || '', eventData.id, language || 'en',
          // Use current UI language
  ...
  const handleDeleteSeries = async () => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.rpc('delete_recurring_series', {
        p_event_id: parentId,
        p_user_id: user?.id,
  ...
  return <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto w-[95vw] sm:w-full">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {initialData && <div className="flex items-center text-sm text-muted-foreground mb-4 rounded-md p-4 py-[8px] px-[8px] bg-gray-900">
                <span className="flex items-center mr-4">
                  <Clock className="mr-1 h-4 w-4" />
                  <span>{t("common.created")} {new Date(initialData.created_at).toLocaleString(language)}</span>
                </span>
                <span className="flex items-center">
                  <RefreshCcw className="mr-1 h-4 w-4" />
                  <span>{t("common.lastUpdated")} {new Date(initialData.created_at).toLocaleString(language)}</span>
                </span>
              </div>}
            ...
            <div className="flex flex-col sm:flex-row gap-2 pt-4">
              ...
              {(eventId || initialData) && <Button type="button" variant="destructive" onClick={() => setShowDeleteDialog(true)} disabled={isLoading} className="flex-1 sm:flex-none">
                  {t("common.delete")}
                </Button>}
            </div>
          </form>
      ...
      <RecurringDeleteDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog} onDeleteThis={handleDeleteThis} onDeleteSeries={handleDeleteSeries} isRecurringEvent={isRecurringEvent} isLoading={isLoading} />
    </>;
};