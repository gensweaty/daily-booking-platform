...
export const EventReminderNotifications = () => {
  ...
  const { data: events } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .lte('reminder_time', futureWindow.toISOString())
        .is('deleted_at', null)
  ...
  // Send email reminder using existing booking confirmation email function
  const sendEmailReminder = async (event: any) => {
    try {
      if (!event.social_network_link) {
        console.log("📧 No email address for event reminder, skipping email");
        return false;
      }
      ...
      const { data, error } = await supabase.functions.invoke('send-booking-approval-email', {
        body: { 
          eventId: event.id,
          isReminder: true // This will add [Reminder] prefix to subject
        }
      });
  ...
  const processDueReminders = async (eventsToCheck: any[]) => {
    ...
    try {
      ...
      for (const event of eventsToCheck) {
        ...
        const reminderKey = `${event.id}-${event.reminder_time}`;
