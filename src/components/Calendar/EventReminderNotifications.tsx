...
export const EventReminderNotifications = () => {
  ...
  const showDashboardNotification = (eventTitle: string) => {
    ...
    toast({
      ...
      description: `${t('calendar.eventReminder')}: ${eventTitle}`,
      duration: 8000,
      action: (
  ...
  const sendEmailReminder = async (event: any) => {
    try {
      ...
      toast({
        ...
        description: t("calendar.reminderEmailSent"),
        duration: 3000,
      });
    ...
    try {
      ...
      for (const event of eventsToCheck) {
        ...
        if (isDue && !processedReminders.has(reminderKey)) {
          console.log('🔔 PROCESSING EVENT REMINDER:', event.title);
          ...
          // Show dashboard notification
          showDashboardNotification(event.title || event.user_surname);
          
          // Show system notification
          const result = await platformNotificationManager.createNotification({
            title: "📅 Event Reminder",
            body: `${t('calendar.eventReminder')}: ${event.title || event.user_surname}`,
            icon: "/favicon.ico",
          ...
          // Send email reminder if enabled and has email
          ...
          console.log('📧 Email reminder:', event.email_reminder_enabled && event.social_network_link ? '✅ Enabled' : '❌ Disabled');
