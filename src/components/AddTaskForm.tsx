...
const AddTaskForm = ({ onClose, editingTask, boardUserId, externalUserName, username }: AddTaskFormProps) => {
      ...
      const taskData = {
        ...
        ...(externalUserName ? {
          ...
          ...(editingTask ? {
            ...
            last_edited_by_name: username || 'Admin',
            last_edited_at: new Date().toISOString()
          } : {
            // Creating new task
  ...
  return (
    <div className={`w-full overflow-x-hidden max-w-full [word-break:break-word] ${isMobile ? 'space-y-2 px-0 pb-1' : 'space-y-3 sm:space-y-6 p-2 sm:p-4'}`}>
