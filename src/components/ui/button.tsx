  ...
  ({ className, variant, size, asChild = false, children, ...props }, ref) => {
    ...
    const wrappedChildren = isGeorgian && typeof children === 'string' ? (
      <GeorgianAuthText fontWeight={props.disabled ? 'normal' : 'bold'}>
        {children === "დაჯავშნე ახლა" ? "დაჯავშნე" : children}
      </GeorgianAuthText>
    ) : children;
    
    // Use the getGeorgianFontStyle utility for consistent styling
    const georgianStyle = isGeorgian ? getGeorgianFontStyle() : {};
