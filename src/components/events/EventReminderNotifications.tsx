  ...
  const sendEmailReminder = async (event: any) => {
    try {
      ...
      if (!recipientEmail || !emailRegex.test(recipientEmail)) {
        console.warn("❌ No valid email for event reminder:", event.title);
        return false;
      }

      console.log("📧 Sending email reminder for event:", event.title, "to:", recipientEmail);
      
      const { data, error } = await supabase.functions.invoke('send-booking-approval-email', {
        body: {
          recipientEmail,
          fullName: event.user_surname || event.title,
      ...
      // Update database - mark as sent and disable
      await supabase
        .from('events')
        .update({
    ...
    try {
      ...
      for (const event of eventsToCheck) {
        ...
        if (isDue && !processedReminders.has(reminderKey)) {
          ...
          console.log('📧 Email reminder: ✅ Processed');
