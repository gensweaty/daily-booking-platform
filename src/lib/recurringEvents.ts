...
export const getRepeatOptions = (selectedDate?: Date, t?: (key: string, params?: Record<string, string | number>) => string): RepeatOption[] => {
  ...
  if (selectedDate && t) {
    ...
    const language = t("recurring.weekly") === "ყოველკვირეული" ? "ka" : 
    ...
    // Build dynamic labels based on language using translation keys with parameters
    options.push(
      { 
        value: "weekly", 
      ...
      { 
        value: "monthly", 
        label: t("recurring.monthlyOnDay", { day: dayOfMonth.toString() })
      },
      { 
        value: "yearly", 
        label: t("recurring.annuallyOn", { month: monthLabel, day: dayOfMonth.toString() })
      }
    );
