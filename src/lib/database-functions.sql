...
-- ✅ FIX: Update save_event_with_persons function to handle reminder fields properly
...
BEGIN
  ...
  ELSE
    RAISE NOTICE '❌ pg_cron extension is NOT available - please enable it in Supabase Dashboard > Database > Extensions';
    RETURN;
  END IF;
END $$;
...
BEGIN
  ...
  ELSE
    RAISE NOTICE '❌ pg_net extension is NOT available - please enable it in Supabase Dashboard > Database > Extensions';
    RETURN;
  END IF;
END $$;
...
-- ✅ FIX: Add debugging function to check current reminder events
