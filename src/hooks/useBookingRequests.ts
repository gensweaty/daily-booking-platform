...
export const useBookingRequests = () => {
  ...
  const approveMutation = useMutation({
    mutationFn: async (bookingId: string) => {
      ...
      // Process files from booking request to event using our centralized function
      try {
        // This will handle all file transfer logic in a consistent way
        const createdFiles = await associateBookingFilesWithEvent(booking.id, eventData.id, user.id);
        console.log(`Successfully associated ${createdFiles.length} files with event`);
      } catch (error) {
