...
export const useEventDialog = ({
  ...
  const handleCreateEvent = async (data: Partial<CalendarEventType>) => {
    try {
      ...
      const eventData = {
        ...data,
        // Make sure event type is properly set - if no type specified, default to 'event'
        ...
        // Set flag to ensure customer creation for regular events
        // Skip creating customer only if event comes from CRM (has customer_id)
        shouldCreateCustomer: data.customer_id ? false : true,
  ...
  const handleUpdateEvent = async (data: Partial<CalendarEventType>) => {
    try {
      ...
      const eventData = {
        ...data,
        type: selectedEvent.type || data.type || 'event',
        ...
        // Only set shouldCreateCustomer flag if it's an update from calendar to customer
        // If the event already has a customer_id, don't try to create a new customer
        shouldCreateCustomer: selectedEvent.customer_id ? false : true
      };
