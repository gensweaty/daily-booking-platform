...
export const useCombinedEvents = (businessId?: string) => {
  ...
  const { data: directEvents, isLoading: isLoadingDirect, error: directError } = useQuery({
    ...
    staleTime: 100, // 100ms - extremely short stale time for more frequent refreshes
  ...
  const { data: allRequests, isLoading: isLoadingRequests, error: requestsError } = useQuery({
    ...
    refetchOnMount: true,
    refetchOnWindowFocus: true,
    staleTime: 100, // 100ms - extremely short stale time
    refetchInterval: 2000, // Refresh every 2 seconds
  });
  ...
  useEffect(() => {
    ...
    if (directEvents || allRequests) {
      ...
      console.log(`[useCombinedEvents] Processing ${directEvents?.length || 0} direct events and ${allRequests?.length || 0} requests`);
