...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    try {
      // Get the currency symbol based on language - ADD ADDITIONAL LOGGING
      ...
      if (paymentStatus) {
        ...
        if ((paymentStatus === 'partly_paid' || paymentStatus === 'partly') && paymentAmount !== undefined && paymentAmount !== null) {
          ...
          console.log(`Formatting partly paid amount with currency: ${amountDisplay}`);
          ...
          console.log(`Formatting fully paid amount with currency: ${amountDisplay}`);
          paymentInfo = `<p><strong>Payment status:</strong> ${formattedStatus}${amountDisplay}</p>`;
        } else {
      ...
      return new Response(
        JSON.stringify({ 
          ...
          paymentStatus: paymentStatus, // Log payment status used
          paymentAmount: paymentAmount // Log payment amount used
        }),
        { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" }}
      );
