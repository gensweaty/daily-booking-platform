import { TranslationType } from './types';

export const ka: TranslationType = {
  common: {
    success: "წარმატება",
    error: "შეცდომა",
    errorOccurred: "მოხდა შეცდომა",
    successMessage: "ოპერაცია წარმატებით შესრულდა",
    copiedToClipboard: "კოპირებულია ბუფერში",
    deleteSuccess: "წარმატებით წაიშალა",
    deleteError: "შეცდომა წაშლისას",
    missingUserInfo: "აკლია მომხმარებლის ინფორმაცია",
    warning: "გაფრთხილება",
    cancel: "გაუქმება",
    submitting: "იგზავნება...",
    create: "შექმნა",
    update: "განახლება",
    delete: "წაშლა",
    and: "და",
    loading: "იტვირთება...",
    rateLimitReached: "ლიმიტი მიღწეულია",
    waitBeforeBooking: "გთხოვთ დაელოდოთ {{time}} სანამ გააგზავნით სხვა მოთხოვნას",
    rateLimitMessage: "შეგიძლიათ მხოლოდ ერთი ჯავშნის მოთხოვნა ყოველ 2 წუთში",
    waitTimeRemaining: "გთხოვთ დაელოდოთ",
    of: "დან",
    attachments: "დანართები",
    refreshing: "მონაცემები განახლდება...",
    copyError: "შეცდომა ტექსტის კოპირებისას",
    new: "ახალი",
    request: "მოთხოვნა",
    requests: "მოთხოვნები",
    backToHome: "მთავარ გვერდზე დაბრუნება",
    supportedFormats: ".jpg, .jpeg, .png, .pdf, .docx, .xls",
    deleteConfirmMessage: "ეს მოქმედება ვერ გაუქმდება. დარწმუნებული ხართ, რომ გსურთ ამ ელემენტის წაშლა?",
    noDescription: "აღწერა არ არის მითითებული",
    deleteConfirmTitle: "დაადასტურეთ წაშლა?",
    authRequired: "საჭიროა ავტორიზაცია",
    save: "შენახვა",
    saving: "ინახება..."
  },
  events: {
    eventCreated: "ღონისძიება წარმატებით შეიქმნა",
    eventUpdated: "ღონისძიება წარმატებით განახლდა",
    eventDeleted: "ღონისძიება წარმატებით წაიშალა",
    submitBookingRequest: "ჯავშნის მოთხოვნის გაგზავნა",
    fullNameRequired: "საჭიროა სრული სახელი",
    fullName: "სრული სახელი",
    phoneNumber: "ტელეფონის ნომერი",
    paymentStatus: "გადახდის სტატუსი",
    selectPaymentStatus: "აირჩიეთ გადახდის სტატუსი",
    paymentAmount: "გადახდის ოდენობა",
    startDateTime: "დაწყების თარიღი და დრო",
    endDateTime: "დასრულების თარიღი და დრო",
    eventNotes: "შენიშვნები",
    addEventNotes: "დაამატეთ შენიშვნები თქვენს ჯავშანზე",
    editEvent: "ღონისძიების რედაქტირება",
    addNewEvent: "ახალი ღონისძიების დამატება",
    updateEvent: "ღონისძიების განახლება",
    createEvent: "ღონისძიების შექმნა",
    deleteEvent: "ღონისძიების წაშლა",
    timeSlotUnavailable: "დროის ინტერვალი მიუწვდომელია",
    timeSlotConflict: "განრიგის კონფლიქტი",
    timeSlotNotAvailable: "ეს დროის ინტერვალი არ არის ხელმისაწვდომი",
    socialLinkEmail: "ელ.ფოსტა / სოციალური ბმული",
    dateAndTime: "თარიღი და დრო",
    date: "თარიღი",
    time: "დრო",
    start: "დაწყება",
    end: "დასრულება",
    eventDetails: "ღონისძიების დეტალები",
    title: "ღონისძიების სათაური",
    bookAppointment: "დაჯავშნა",
    submitRequest: "მოთხოვნის გაგზავნა"
  },
  bookings: {
    approvedBooking: "ჯავშნის მოთხოვნა დადასტურებულია",
    requestApproved: "ჯავშნის მოთხოვნა დადასტურებულია",
    requestRejected: "ჯავშნის მოთხოვნა უარყოფილია",
    requestDeleted: "ჯავშნის მოთხოვნა წაშლილია",
    newRequest: "ჯავშნის მოთხოვნა",
    pendingRequestsCount: "თქვენ გაქვთ {count} ახალი ჯავშნის მოთხოვნა",
    requestSubmitted: "ჯავშნის მოთხოვნა გაიგზავნა",
    requestSubmittedDescription: "დადასტურება მოგივათ მეილზე"
  },
  tasks: {
    taskAdded: "დავალება დამატებულია",
    taskUpdated: "დავალება განახლებულია",
    taskDeleted: "დავალება წაშლილია",
    title: "დავალებების მართვა",
    description: "დარჩით ორგანიზებული და პროდუქტიული",
    feature1: "Kanban დაფა ვიზუალური დავალებების ორგანიზებისთვის",
    feature2: "სტატუსის თვალყურის დევნება და პროგრესის მონიტორინგი",
    feature3: "ეფექტური დავალებების პრიორიტეტიზაცია",
    feature4: "მარტივი გადათრევის ფუნქციონალი",
    feature5: "სწრაფი შენიშვნების შენახვა დავალებებისთვის",
    addTask: "დავალების დამატება",
    editTask: "დავალების რედაქტირება",
    titlePlaceholder: "დავალების სათაური",
    descriptionLabel: "აღწერილობა",
    todo: "გასაკეთებელი",
    inProgress: "მიმდინარეობს",
    done: "დასრულებული",
    deleteTaskConfirmTitle: "წავშალოთ დავალება?"
  },
  crm: {
    customerCreated: "მომხმარებელი დამატებულია",
    customerUpdated: "მომხმარებელი განახლებულია",
    customerDeleted: "მომხმარებელი წაშლილია",
    title: "მომხმარებლების მართვა",
    description: "ააშენეთ და შეინარჩუნეთ ძლიერი ურთიერთობები კლიენტებთან ჩვენი CRM-ით",
    feature1: "ცენტრალიზებული მომხმარებელთა ინფორმაციის მართვა",
    feature2: "ფაილების დანართები და დოკუმენტების ორგანიზება",
    feature3: "გადახდის თვალყურის დევნება და სტატუსის მონიტორინგი",
    feature4: "ელასტიური ძიება სწრაფი მონაცემთა წვდომისთვის",
    feature5: "ერთი დაწკაპუნებით Excel-ის ჩამოტვირთვა ყველა ნაჩვენები მონაცემისთვის",
    notPaid: "არ არის გადახდილი",
    paidPartly: "ნაწილობრივ გადახდილი",
    paidFully: "სრულად გადახდილი",
    newCustomer: "ახალი მომხმარებელი",
    fullNameRequired: "საჭიროა სრული სახელი",
    fullNamePlaceholder: "შეიყვანეთ მომხმარებლის სრული სახელი",
    phoneNumber: "ტელეფონის ნომერი",
    phoneNumberPlaceholder: "შეიყვანეთ ტელეფონის ნომერი",
    socialLinkEmail: "ელ.ფოსტა / სოციალური ბმული",
    socialLinkEmailPlaceholder: "შეიყვანეთ ელფოსტა ან სოციალური მედიის ბმული",
    createEventForCustomer: "შექმენით ღონისძიება ამ მომხმარებლისთვის",
    customerFromEvent: "ეს მომხმარებელი შეიქმნა ღონისძიებიდან",
    paymentStatus: "გადახდის სტატუსი",
    paymentAmount: "გადახდის თანხა",
    selectPaymentStatus: "აირჩიეთ გადახდის სტატუსი",
    comment: "კომენტარი",
    commentPlaceholder: "დაამატეთ კომენტარი ამ მომხმარებლის შესახებ",
    create: "შექმნა",
    cancel: "გაუქმება",
    addCustomer: "მომხმარებლის დამატება",
    editCustomer: "მომხმარებლის რედაქტირება",
    update: "განახლება",
    dates: "თარიღები",
    actions: "მოქმედებები",
    customersPerPage: "მომხმარებლები თითო გვერდზე",
    noCustomers: "მომხმარებლები არ არის ნაპოვნი",
    noCustomersDescription: "დაამატეთ თქვენი პირველი მომხმარებელი დასაწყებად",
    refreshing: "მონაცემები განახლდება...",
    yes: "დიახ",
    no: "არა",
    copiedToClipboard: "კოპირებულია ბუფერში",
    copyError: "შეცდომა ტექსტის კოპირებისას",
    deleteSuccess: "წარმატებით წაიშალა",
    deleteError: "შეცდომა წაშლისას",
    missingUserInfo: "აკლია მომხმარებლის ინფორმაცია",
    noDataToExport: "არ არის მონაცემები ექსპორტისთვის",
    error: "შეცდომა"
  },
  notes: {
    noteAdded: "შენიშვნა დამატებულია",
    noteAddedDescription: "შენიშვნა წარმატებით დაემატა",
    noteUpdated: "შენიშვნა განახლებულია",
    noteDeleted: "შენიშვნა წაშლილია",
    title: "სათაური",
    titlePlaceholder: "შენიშვნის სათაური",
    titleRequired: "სათაური აუცილებელია",
    content: "შინაარსი",
    contentPlaceholder: "დაწერეთ თქვენი შენიშვნა აქ...",
    addNewNote: "ახალი შენიშვნის დამატება"
  },
  reminders: {
    reminderCreated: "შეხსენება დამატებულია"
  },
  dashboard: {
    exportSuccessful: "ექსპორტი წარმატებით დასრულდა",
    exportSuccessMessage: "მონაცემები წარმატებით ექსპორტირდა",
    welcome: "კეთილი იყოს თქვენი დაბრუნება",
    greeting: "გამარჯობა",
    goodMorning: "დილა მშვიდობისა",
    goodAfternoon: "შუადღე მშვიდობისა",
    goodEvening: "საღამო მშვიდობისა",
    goodNight: "ღამე მშვიდობისა",
    overview: "მიმოხილვა",
    calendar: "კალენდარი",
    bookingCalendar: "ჯავშნების კალენდარი",
    tasks: "დავალებები",
    reminders: "შეხსენებები",
    notes: "შენიშვნები",
    business: "ბიზნესი",
    stats: "სტატისტიკა",
    statistics: "სტატისტიკა",
    crm: "კლიენტები",
    signOut: "გასვლა",
    darkMode: "მუქი რეჟიმი",
    lightMode: "ნათელი რეჟიმი",
    systemMode: "სისტემური",
    more: "მეტი",
    addEvent: "ღონისძიების დამატება",
    day: "დღე",
    week: "კვირა",
    month: "თვე",
    totalTasks: "სულ დავალებები",
    completed: "დასრულებული",
    inProgress: "მიმდინარე",
    todo: "გასაკეთებელი",
    totalEvents: "სულ ღონისძიებები",
    partlyPaid: "ნაწილობრივ გადახდილი",
    fullyPaid: "სრულად გადახდილი",
    totalIncome: "ჯამური შემოსავალი",
    fromAllEvents: "ყველა ღონისძიებიდან",
    changePassword: "პაროლის შეცვლა",
    subtitle: "თქვენი პროდუქტიულობის ცენტრი",
    profile: "პროფილი",
    category: "კატეგორია",
    details: "დეტალები",
    additionalInfo: "დამატებითი ინფორმაცია",
    taskStatistics: "დავალებების სტატისტიკა",
    eventStatistics: "ღონისძიებების სტატისტიკა",
    financialSummary: "ფინანსური შეჯამება",
    summaryStatistics: "შეჯამების სტატისტიკა",
    eventsData: "ღონისძიებების მონაცემები"
  },
  nav: {
    signin: "შესვლა",
    startJourney: "დაიწყეთ თქვენი მოგზაურობა",
    contact: "კონტაქტი"
  },
  hero: {
    title: "გაზარდეთ თქვენი ბიზნესის პროდუქტიულობა SmartBookly-სთან ერთად",
    subtitle: "ყველაფერი-ერთში გადაწყვეტილება - ღონისძიებები, დავალებები, მომხმარებლები და ანალიტიკა",
    description: "აიღეთ სრული კონტროლი თქვენს სამუშაო პროცესზე ჩვენი ჯავშნების კალენდარი, ძლიერი დავალებების მართვის სისტემა, ჭკვიანი მომხმარებელთან ურთიერთობის მართვა და ჩაშენებული ანალიტიკა. ყველაფერი რაც თქვენს ბიზნესს სჭირდება, ერთ ადგილზე."
  },
  features: {
    title: "რას გთავაზობთ",
    mainTitle: "ყველაფერი რაც თქვენს ბიზნესს სჭირდება წარმატებისთვის",
    booking: "ჯავშნები",
    tasks: "დავალებები",
    crm: "კლიენტები",
    analytics: "ანალიტიკა",
    website: "ვებ-გვერდი",
    ownBookingWebsite: "თქვენი საკუთარი ჯავშნების ვებ-გვერდი",
    businessTitle: "ნებისმიერი ტიპის ბიზნესისთვის",
    smartAppointment: "ჭკვიანი შეხვედრების დაგეგმვა",
    automatedAnalytics: "ავტომატიზირებული ანალიტიკა",
    modernCRM: "თანამედროვე CRM გადაწყვეტა",
    kanbanManagement: "Kanban დავალებების მართვა"
  },
  booking: {
    title: "ჭკვიანი ჯავშნების სისტემა",
    description: "ეფექტურად მართეთ თქვენი შეხვედრები და ღონისძიებები",
    feature1: "მრავალმხრივი კალენდარი (თვე, კვირა, დღე)",
    feature2: "ღონისძიებების დაგეგმვა მორგებული დროის ინტერვალებით",
    feature3: "მომხმარებელთა ჯავშნების მართვა გადახდის თვალყურის დევნებით",
    feature4: "ავტომატური ღონისძიებების სინქრონიზაცია CRM-სთან",
    feature5: "ჯავშნების შეტყობინებები ელ-ფოსტით და პანელზე",
    requestSubmitted: "ჯავშნის მოთხოვნა გაიგზავნა",
    requestSubmittedDescription: "თქვენი ჯავშნის მოთხოვნა გაგზავნილია და ელოდება დამტკიცებას",
    yourEmailPlaceholder: "შეიყვანეთ თქვენი ელ-ფოსტა",
    bookAppointment: "დაჯავშნე შეხვედრა",
    requestApproved: "ჯავშნის მოთხოვნა დამტკიცებულია",
    newRequest: "ახალი ჯავშნის მოთხოვნა"
  },
  analytics: {
    title: "ყოვლისმომცველი ანალიტიკა",
    description: "თვალყური ადევნეთ თქვენს მუშაობას და ზრდას",
    feature1: "ჯავშნებისა და შემოსავლის ანალიტიკა",
    feature2: "მორგებული თარიღის დიაპაზონის ფილტრაცია",
    feature3: "თვიდან თვემდე შემოსავლის შედარება",
    feature4: "ინტერაქტიული ვიზუალური მეტრიკები და დიაგრამები",
    feature5: "ერთი დაწკაპუნებით Excel-ის ჩამოტვირთვა"
  },
  website: {
    title: "უფასო ვებგვერდი თქვენი ბიზნესისთვის",
    description: "მიიღეთ პროფესიონალური ჯავშნების ვებგვერდი თქვენი ბიზნესისთვის კოდის გარეშე",
    feature1: "პროფესიონალური დიზაინი",
    feature2: "მობილურზე მორგებული",
    feature3: "სერვისების ჩამონათვალი",
    feature4: "ონლაინ ჯავშნების ინტეგრაცია",
    feature5: "ელფოსტით შეხვედრის დადასტურება"
  },
  cta: {
    title: "მზად ხართ გაზარდოთ თქვენი ბიზნესი?",
    subtitle: "დაიწყეთ ჩვენი პლატფორმის გამოყენება დღესვე და იხილეთ განსხვავება!",
    button: "დაიწყეთ ახლავე"
  },
  footer: {
    copyright: "© 2023 თქვენი კომპანიის სახელი. ყველა უფლება დაცულია.",
    terms: "მომსახურების პირობები",
    privacy: "კონფიდენციალურობის პოლიტიკა",
    description: "ბიზნესის მართვის პლატფორმა სერვისის მიმწოდებლებისთვის",
    navigation: "ნავიგაცია",
    legal: "სამართლებრივი",
    termsAndPrivacy: "პირობები და კონფიდენციალურობა",
    rights: "ყველა უფლება დაცულია"
  },
  business: {
    events: "ღონისძიებები და გართობა",
    health: "ჯანმრთელობა და კეთილდღეობა",
    sports: "სპორტი და ფიტნესი",
    beauty: "სილამაზე და სპა",
    personal: "პერსონალური მომსახურება",
    education: "განათლება და კოუჩინგი",
    eventsDesc: "ღონისძიებების დამგეგმავები, შემსრულებლები, ვენები და სხვა",
    healthDesc: "ექიმები, თერაპევტები, კეთილდღეობის მწვრთნელები",
    sportsDesc: "პირადი მწვრთნელები, სპორტის მწვრთნელები, ფიტნეს ცენტრები",
    beautyDesc: "სალონები, სტილისტები, სპა ცენტრები",
    personalDesc: "კონსულტანტები, ფოტოგრაფები, სახლის მომსახურება",
    educationDesc: "მასწავლებლები, რეპეტიტორები, ტრენერები, ვორქშოპები",
    myBusiness: "ჩემი ბიზნესი",
    businessProfile: "ბიზნეს პროფილი",
    bookingRequests: "ჯავშნის მოთხოვნები",
    pendingRequests: "მიმდინარე მოთხოვნები",
    approvedRequests: "დამტკიცებული მოთხოვნები",
    rejectedRequests: "უარყოფილი მოთხოვნები",
    noPendingRequests: "არ არის მიმდინარე მოთხოვნები",
    noApprovedRequests: "არ არის დამტკიცებული მოთხოვნები",
    noRejectedRequests: "არ არის უარყოფილი მოთხოვნები",
    pendingRequestsDescription: "როდესაც მომხმარებლები გააგზავნიან ჯავშნის მოთხოვნებს, ისინი გამოჩნდება აქ თქვენს დასამტკიცებლად.",
    approvedRequestsDescription: "დამტკიცებული ჯავშნის მოთხოვნები გამოჩნდება აქ.",
    rejectedRequestsDescription: "უარყოფილი ჯავშნის მოთხოვნები გამოჩნდება აქ.",
    editBusinessProfile: "ბიზნეს პროფილის რედაქტირება",
    chooseFile: "აირჩიეთ ფაილი",
    noFileChosen: "ფაილი არ არის არჩეული",
    supportedFormats: "მხარდაჭერილი ფორმატები: სურათები (jpg, png, webp) 50MB-მდე, დოკუმენტები (pdf, docx, xlsx, pptx) 1MB-მდე",
    uploadImageCover: "ატვირთეთ სურათი თქვენი ბიზნესის გარეკანისთვის (JPEG, PNG, WebP)",
    publicPageUrl: "ეს გამოყენებული იქნება თქვენი საჯარო გვერდის URL-ისთვის:",
    notFound: "ბიზნესი ვერ მოიძებნა",
    notFoundDescription: "სამწუხაროდ, ვერ ვიპოვეთ ბიზნესი ამ URL-ით. გთხოვთ, შეამოწმოთ URL და სცადოთ ხელახლა.",
    viewPublicPage: "საჯარო გვერდის ნახვა",
    customer: "მომხმარებელი",
    title: "სათაური",
    dateTime: "თარიღი და დრო",
    actions: "მოქმედებები",
    approve: "დამტკიცება",
    reject: "უარყოფა",
    delete: "წაშლა",
    deleteBookingRequest: "წავშალოთ ჯავშნის მოთხოვნა?",
    deleteConfirmation: "დარწმუნებული ხართ, რომ გსურთ ამ ჯავშნის მოთხოვნის წაშლა? ეს მოქმედება ვერ გაუქმდება.",
    businessName: "ბიზნესის სახელი",
    urlSlug: "URL სეგმენტი",
    description: "აღწერა",
    coverPhoto: "გარეკანის ფოტო",
    phone: "ტელეფონი",
    email: "ელ-ფოსტა",
    website: "ვებსაიტი",
    address: "მისამართი",
    updateProfile: "პროფილის განახლება",
    availableTimes: "ხელმისაწვდომი დროები",
    clickToRequest: "დააწკაპუნეთ ნებისმიერ დროზე ჯავშნის მოთხოვნისთვის",
    contactInformation: "საკონტაქტო ინფორმაცია"
  },
  auth: {
    welcome: "კეთილი იყოს თქვენი მობრძანება SmartBookly-ში",
    description: "მართეთ თქვენი ბიზნესი ჩვენი ყველაფერი-ერთში გადაწყვეტილებით",
    signInButton: "შესვლა",
    signUpButton: "რეგისტრაცია",
    emailLabel: "ელ-ფოსტა",
    passwordLabel: "პაროლი",
    usernameLabel: "მომხმარებლის სახელი",
    forgotPassword: "დაგავიწყდათ პაროლი?",
    noAccount: "არ გაქვთ ანგარიში?",
    haveAccount: "უკვე გაქვთ ანგარიში?",
    signInCta: "შესვლა",
    signUpCta: "რეგისტრაცია",
    agreeToTerms: "ვეთანხმები პირობებს",
    requiredField: "ეს ველი აუცილებელია",
    invalidEmail: "გთხოვთ შეიყვანოთ სწორი ელ-ფოსტა",
    passwordMinLength: "პაროლი უნდა შეიცავდეს მინიმუმ 6 სიმბოლოს",
    passwordMatch: "პაროლები არ ემთხვევა",
    confirmPassword: "დაადასტურეთ პაროლი",
    resetPassword: "პაროლის აღდგენა",
    resetPasswordCta: "პაროლის აღდგენის ბმულის გაგზავნა",
    resetPasswordSuccess: "პაროლის აღდგენის ელ-ფოსტა გაგზავნილია",
    resetPasswordError: "შეცდომა პაროლის აღდგენის ელ-ფოსტის გაგზავნისას",
    signInError: "არასწორი ელ-ფოსტა ან პაროლი",
    signUpError: "შეცდომა ანგარიშის შექმნისას",
    businessNameLabel: "ბიზნესის სახელი",
    businessTypeLabel: "ბიზნესის ტიპი",
    selectBusinessType: "აირჩიეთ ბიზნესის ტიპი",
    fullNameLabel: "სრული სახელი",
    fullName: "სრული სახელი",
    phoneLabel: "ტელეფონი",
    terms: "პირობები",
    privacy: "კონფიდენციალურობა",
    and: "და",
    resetInstructions: "შეიყვანეთ თქვენი ელ-ფოსტა და ჩვენ გამოგიგზავნით ინსტრუქციებს პაროლის აღსადგენად.",
    passwordRequirements: "პაროლი უნდა შეიცავდეს მინიმუმ 6 სიმბოლოს",
    confirmPasswordLabel: "დაადასტურეთ პაროლი",
    signingIn: "შესვლა...",
    signingUp: "რეგისტრაცია...",
    loading: "იტვირთება...",
    enterEmail: "შეიყვანეთ თქვენი ელ-ფოსტა",
    redeemCode: "გამოსყიდვის კოდი",
    redeemCodeOptional: "გამოსყიდვის კოდი (არასავალდებულო)",
    enterRedeemCode: "შეიყვანეთ თქვენი გამოსყიდვის კოდი",
    sendResetLink: "პაროლის აღდგენის ბმულის გაგზავნა",
    backToSignIn: "დაბრუნება შესვლაზე",
    passwordsDoNotMatch: "პაროლები არ ემთხვევა",
    passwordTooShort: "პაროლი ძალიან მოკლეა",
    verificationRequired: "მნიშვნელოვანი: რეგისტრაციის შემდეგ, თქვენ უნდა დააწკაპუნოთ თქვენს ელ-ფოსტაზე გამოგზავნილ ვერიფიკაციის ბმულზე თქვენი ანგარიშის გასააქტიურებლად.",
    checkEmailFolders: "გთხოვთ, შეამოწმოთ როგორც თქვენი შემოსული, ასევე სპამის საქაღალდეები დადასტურების ელ-ფოსტისთვის."
  },
  contact: {
    email: "ელ-ფოსტა",
    getInTouch: "დაგვიკავშირდით",
    contactInfo: "საკონტაქტო ინფორმაცია",
    phone: "ტელეფონი",
    address: "მისამართი",
    addressLine1: "123 ბიზნეს ქუჩა",
    addressLine2: "ქალაქი, ქვეყანა",
    businessHours: "სამუშაო საათები",
    workingHours: "ორშაბათი-პარასკევი: 9სთ-17სთ",
    weekendHours: "შაბათი-კვირა: დასვენების დღე",
    sendMessage: "გამოგვიგზავნეთ შეტყობინება",
    name: "სახელი",
    namePlaceholder: "შეიყვანეთ თქვენი სახელი",
    emailPlaceholder: "შეიყვანეთ თქვენი ელ-ფოსტა",
    message: "შეტყობინება",
    messagePlaceholder: "დაწერეთ თქვენი შეტყობინება აქ",
    send: "გაგზავნა",
    sending: "იგზავნება...",
    messageSent: "შეტყობინება გაგზავნილია",
    messageSentDesc: "გმადლობთ, თქვენი შეტყობინება მიღებულია",
    error: "შეცდომა",
    errorDesc: "შეტყობინების გაგზავნისას მოხდა შეცდომა. გთხოვთ, სცადოთ ხელახლა."
  },
  calendar: {
    attachment: "დანართი",
    day: "დღე",
    week: "კვირა",
    month: "თვე",
    addEvent: "ღონისძიების დამატება",
    bookNow: "დაჯავშნე ახლავე",
    weekOf: "კვირა"
  },
  legal: {
    title: "წესები და კონფიდენციალურობის პოლიტიკა",
    lastUpdated: "ბოლოს განახლდა: {{date}}",
    termsOfService: "მომსახურების პირობები",
    welcomeText: "კეთილი იყოს თქვენი მობრძანება SmartBookly-ში. ეს მომსახურების პირობები ('პირობები') არეგულირებს ჩვენი SaaS პლატფორმისა და სერვისების ('სერვისები') გამოყენებას. ჩვენი სერვისების წვდომით ან გამოყენებით თქვენ ეთანხმებით ამ პირობებს. თუ არ ეთანხმებით, გთხოვთ, არ გამოიყენოთ ჩვენი სერვისები.",
    generalInfo: {
      title: "1. ზოგადი ინფორმაცია",
      companyName: "კომპანიის სახელი: SmartBookly",
      registered: "რეგისტრირებულია: საქართველოში",
      contact: "საკონტაქტო ელ-ფოსტა: info@smartbookly.com"
    },
    eligibility: {
      title: "2. უფლებამოსილება",
      description: "თქვენ უნდა იყოთ მინიმუმ 18 წლის, რომ გამოიყენოთ ჩვენი სერვისები. ჩვენი პლატფორმის გამოყენებით თქვენ ადასტურებთ, რომ აკმაყოფილებთ ამ მოთხოვნას."
    },
    accountSecurity: {
      title: "3. ანგარიშის რეგისტრაცია და უსაფრთხოება",
      requirements: [
        "ანგარიშის შექმნისას უნდა მიაწოდოთ ზუსტი და სრული ინფორმაცია.",
        "თქვენ პასუხისმგებელი ხართ თქვენი ანგარიშისა და პაროლის კონფიდენციალურობის შენარჩუნებაზე.",
        "დაუყოვნებლივ გვაცნობეთ თქვენს ანგარიშზე არაავტორიზებული წვდომის შესახებ."
      ]
    }
  }
};
