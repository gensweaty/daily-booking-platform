import { TranslationType } from "./types";

export const ka: TranslationType = {
  nav: {
    signin: "შესვლა",
    startJourney: "დაიწყე მოგზაურობა",
    contact: "კონტაქტი"
  },
  hero: {
    title: "მართეთ თქვენი ბიზნესი ჩვენი",
    subtitle: "სრული გადაწყვეტილებით",
    description: "სრული პლატფორმა სერვისის მიმწოდებლებისთვის. კალენდარი, CRM, დავალებების მართვა, სტატისტიკა და დაჯავშნის ვებგვერდი, ყველაფერი ერთ ადგილას!"
  },
  features: {
    title: "ფუნქციები",
    mainTitle: "ყველაფერი, რაც თქვენს ბიზნესს წარმატებისთვის სჭირდება",
    booking: "დაჯავშნა",
    tasks: "დავალებები",
    crm: "CRM",
    analytics: "ანალიტიკა",
    website: "ვებგვერდი",
    ownBookingWebsite: "თქვენი საკუთარი დაჯავშნის ვებგვერდი",
    businessTitle: "ნებისმიერი ტიპის ბიზნესისთვის"
  },
  booking: {
    title: "ჭკვიანი დაჯავშნის სისტემა",
    description: "ეფექტურად მართეთ თქვენი შეხვედრები და ღონისძიებები",
    feature1: "კალენდრის მრავალი ხედი (თვე, კვირა, დღე)",
    feature2: "ღონისძიებების დაგეგმვა მორგებადი დროის სლოტებით",
    feature3: "კლიენტების დაჯავშნის მართვა გადახდის თვალყურის დევნებით",
    feature4: "ავტომატური სინქრონიზაცია CRM-თან",
    feature5: "თვიური, კვირეული და დღიური კალენდარი",
    feature6: "დაჯავშნის შეტყობინებები ელფოსტით და დაფაზე",
    requestSubmitted: "თქვენი მოთხოვნა წარმატებით გაიგზავნა!",
    yourEmailPlaceholder: "შეიყვანეთ თქვენი ელ-ფოსტა"
  },
  analytics: {
    title: "ყოვლისმომცველი ანალიტიკა",
    description: "თვალყური ადევნეთ თქვენს შესრულებას და ზრდას",
    feature1: "დაჯავშნისა და შემოსავლების ანალიტიკა",
    feature2: "თარიღების დიაპაზონის მორგება",
    feature3: "შემოსავლების შედარება თვეების მიხედვით",
    feature4: "ინტერაქტიული ვიზუალური მეტრიკა და გრაფიკები",
    feature5: "Excel-ში ჩამოტვირთვა ერთი დაწკაპუნებით"
  },
  crm: {
    title: "ურთიერთობა კლიენტებთან",
    description: "ააწყვეთ და შეინარჩუნეთ მტკიცე ურთიერთობები კლიენტებთან ჩვენი CRM-ით",
    feature1: "ცენტრალიზებული ინფორმაციის მართვა კლიენტების შესახებ",
    feature2: "ფაილების მიმაგრება და დოკუმენტების ორგანიზება",
    feature3: "გადახდის თვალყურის დევნება და სტატუსის მონიტორინგი",
    feature4: "ელასტიური ძიება მონაცემებზე სწრაფი წვდომისთვის",
    feature5: "ყველა ნაჩვენები მონაცემის Excel-ში ჩამოტვირთვა ერთი დაწკაპუნებით",
    notPaid: "გადაუხდელი",
    paidPartly: "ნაწილობრივ გადახდილი",
    paidFully: "სრულად გადახდილი",
    newCustomer: "ახალი კლიენტი",
    fullNameRequired: "Სრული სახელი",
    fullNamePlaceholder: "შეიყვანეთ კლიენტის სრული სახელი",
    phoneNumber: "Ტელეფონის ნომერი",
    phoneNumberPlaceholder: "შეიყვანეთ ტელეფონის ნომერი",
    socialLinkEmail: "ელფოსტა / სოციალური ქსელის ბმული",
    socialLinkEmailPlaceholder: "შეიყვანეთ ელფოსტა ან სოციალური მედიის ბმული",
    createEventForCustomer: "ღონისძიების შექმნა ამ კლიენტისთვის",
    customerFromEvent: "ეს კლიენტი შეიქმნა ღონისძიებიდან",
    paymentStatus: "Გადახდის სტატუსი",
    paymentAmount: "გადახდის თანხა",
    selectPaymentStatus: "აირჩიეთ გადახდის სტატუსი",
    comment: "კომენტარი",
    commentPlaceholder: "დაამატეთ კომენტარი ამ კლიენტის შესახებ",
    create: "Შექმნა",
    cancel: "გაუქმება",
    addCustomer: "კლიენტის დამატება",
    editCustomer: "კლიენტის რედაქტირება",
    update: "განახლება",
    dates: "თარიღები",
    actions: "ქმედებები",
    customersPerPage: "კლიენტები თითო გვერდზე",
    noCustomers: "კლიენტები ვერ მოიძებნა",
    noCustomersDescription: "დაამატეთ თქვენი პირველი კლიენტი დასაწყებად",
    refreshing: "მონაცემების განახლება...",
    yes: "დიახ",
    no: "არა",
    copiedToClipboard: "დაკოპირებულია ბუფერში",
    copyError: "ტექსტის კოპირების შეცდომა",
    deleteSuccess: "წარმატებით წაიშალა",
    deleteError: "წაშლის შეცდომა",
    missingUserInfo: "მომხმარებლის ინფორმაცია აკლია",
    customerCreated: "კლიენტი წარმატებით შეიქმნა",
    customerUpdated: "კლიენტი წარმატებით განახლდა",
    noDataToExport: "მონაცემები ექსპორტისთვის არ არის",
    error: "შეცდომა"
  },
  tasks: {
    title: "დავალებების მართვა",
    description: "იყავით ორგანიზებული და პროდუქტიული",
    feature1: "Kanban დაფა ვიზუალური დავალებების ორგანიზებისთვის",
    feature2: "დავალების სტატუსის თვალყურის დევნება და პროგრესის მონიტორინგი",
    feature3: "დავალების ეფექტური პრიორიტიზაცია",
    feature4: "მარტივი გადატანისა და ჩამოგდების ფუნქციონირება",
    feature5: "სწრაფი შენიშვნების შენახვა დავალებებისთვის",
    addTask: "დავალების დამატება",
    editTask: "დავალების რედაქტირება",
    titlePlaceholder: "დავალების სათაური",
    descriptionLabel: "აღწერა",
    todo: "გასაკეთებელი",
    inProgress: "მიმდინარე",
    done: "დასრულებული"
  },
  website: {
    title: "უფასო ვებსაიტი თქვენი ბიზნესისთვის",
    description:
      "მიიღეთ პროფესიონალური დაჯავშნის ვებსაიტი თქვენი ბიზნესისთვის ყოველგვარი კოდის გარეშე.",
    feature1: "პროფესიონალური დიზაინი",
    feature2: "მობილურზე რეაგირებადი",
    feature3: "სერვისების ჩამონათვალი",
    feature4: "ონლაინ დაჯავშნის ინტეგრაცია",
    feature5: "ელფოსტის შეხვედრის დამტკიცება",
  },
  cta: {
    title: "მზად ხართ თქვენი ბიზნესის გასაზრდელად?",
    subtitle: "დაიწყეთ ჩვენი პლატფორმის გამოყენება დღესვე და ნახეთ განსხვავება!",
    button: "Დაიწყე ახლა",
  },
  footer: {
    copyright: "© 2023 თქვენი კომპანიის სახელი. Ყველა უფლება დაცულია.",
    terms: "მომსახურების პირობები",
    privacy: "კონფიდენციალურობის პოლიტიკა",
    description: "ბიზნესის მართვის პლატფორმა სერვისის პროვაიდერებისთვის",
    navigation: "ნავიგაცია",
    legal: "იურიდიული",
    termsAndPrivacy: "პირობები და კონფიდენციალურობა",
    rights: "Ყველა უფლება დაცულია",
  },
  business: {
    events: "ღონისძიებები და გართობა",
    health: "ჯანმრთელობა და კეთილდღეობა",
    sports: "სპორტი და ფიტნესი",
    beauty: "სილამაზე და სპა",
    personal: "პერსონალური სერვისები",
    education: "განათლება და ქოუჩინგი",
    eventsDesc: "ღონისძიებების დამგეგმავები, შემსრულებლები, ადგილები და სხვა",
    healthDesc: "ექიმები, თერაპევტები, ველნეს ქოუჩები",
    sportsDesc: "პერსონალური მწვრთნელები, სპორტული მწვრთნელები, ფიტნეს ცენტრები",
    beautyDesc: "სილამაზის სალონები, დალაქები, მანიკურის სალონები, სპა ცენტრები",
    personalDesc: "კონსულტანტები, ფოტოგრაფები, სახლის სერვისები",
    educationDesc: "მასწავლებლები, რეპეტიტორები, ლაიფ ქოუჩები, სემინარების პროვაიდერები",
    myBusiness: "Ჩემი ბიზნესი",
    businessProfile: "Ბიზნეს პროფილი",
    bookingRequests: "დაჯავშნის მოთხოვნები",
    pendingRequests: "მოლოდინში მოთხოვნები",
    approvedRequests: "დამტკიცებული მოთხოვნები",
    rejectedRequests: "უარყოფილი მოთხოვნები",
    noPendingRequests: "მოლოდინში მოთხოვნები არ არის",
    noApprovedRequests: "დამტკიცებული მოთხოვნები არ არის",
    noRejectedRequests: "უარყოფილი მოთხოვნები არ არის",
    pendingRequestsDescription: "როდესაც მომხმარებლები აგზავნიან დაჯავშნის მოთხოვნებს, ისინი გამოჩნდება აქ თქვენი დასამტკიცებლად.",
    approvedRequestsDescription: "დამტკიცებული დაჯავშნის მოთხოვნები გამოჩნდება აქ.",
    rejectedRequestsDescription: "უარყოფილი დაჯავშნის მოთხოვნები გამოჩნდება აქ.",
    editBusinessProfile: "Ბიზნეს პროფილის რედაქტირება",
    viewPublicPage: "საჯარო გვერდის ნახვა",
    businessName: "Ბიზნესის სახელი",
    urlSlug: "URL სლოგი",
    description: "აღწერა",
    coverPhoto: "Ხელმისაწვდომი ფოტო",
    phone: "ტელეფონი",
    email: "ელფოსტა",
    website: "ვებსაიტი",
    address: "მისამართი",
    customer: "მომხმარებელი",
    title: "სათაური",
    dateTime: "თარიღი და დრო",
    actions: "ქმედებები",
    approve: "დამტკიცება",
    reject: "უარყოფა",
    delete: "წაშლა",
    deleteBookingRequest: "დაჯავშნის მოთხოვნის წაშლა",
    deleteConfirmation: "დარწმუნებული ხართ, რომ გსურთ ამ დაჯავშნის მოთხოვნის წაშლა? ამ მოქმედების გაუქმება შეუძლებელია.",
    chooseFile: "აირჩიეთ ფაილი",
    noFileChosen: "ფაილი არ არის არჩეული",
    supportedFormats: "მხარდაჭერილი ფორმატები: სურათები (jpg, png, webp) 50 მბ-მდე, დოკუმენტები (pdf, docx, xlsx, pptx) 1 მბ-მდე",
    uploadImageCover: "ატვირთეთ სურათი თქვენი ბიზ��ესის ყდისთვის (JPEG, PNG, WebP)",
    publicPageUrl: "ეს გამოყენებული იქნება თქვენი საჯარო გვერდის URL-ისთვის:",
    notFound: "ბიზნესი ვერ მოიძებნა",
    notFoundDescription: "უკაცრავად, ამ URL-ით ბიზნესი ვერ ვიპოვეთ. გთხოვთ, შეამოწმოთ URL და სცადოთ ხელახლა.",
    updateProfile: "პროფილის განახლება",
    availableTimes: "ხელმისაწვდომი დრო",
    clickToRequest: "დააწკაპუნეთ ნებისმიერ დროს დაჯავშნის მოთხოვნის გასაგზავნად",
    contactInformation: "საკონტაქტო ინფორმაცია"
  },
  dashboard: {
    welcome: "კეთილი იყოს თქვენი მობრძანება",
    greeting: "გამარჯობა",
    goodMorning: "დილა მშვიდობისა",
    goodAfternoon: "კარგ დღეს გისურვებთ",
    goodEvening: "საღამო მშვიდობისა",
    overview: "Მიმოხილვა",
    calendar: "Კალენდარი",
    bookingCalendar: "დაჯავშნის კალენდარი",
    tasks: "Დავალებები",
    reminders: "შეხსენებები",
    notes: "შენიშვნები",
    business: "ბიზნესი",
    stats: "სტატისტიკა",
    statistics: "სტატისტიკა",
    crm: "CRM",
    signOut: "Გამოსვლა",
    darkMode: "Მუქი რეჟიმი",
    lightMode: "Ნათელი რეჟიმი",
    systemMode: "Სისტემა",
    more: "მეტი",
    addEvent: "ღონისძიების დამატება",
    day: "დღე",
    week: "კვირა",
    month: "თვე",
    totalTasks: "სულ დავალებები",
    completed: "დასრულებული",
    inProgress: "მიმდინარე",
    todo: "გასაკეთებელი",
    totalEvents: "სულ ღონისძიებები",
    partlyPaid: "ნაწილობრივ გადახდილი",
    fullyPaid: "სრულად გადახდილი",
    totalIncome: "სულ შემოსავალი",
    fromAllEvents: "ყველა ღონისძიებიდან",
    exportSuccessful: "ექსპორტი წარმატებულია",
    exportSuccessMessage: "თქვენი მონაცემები წარმატებით ექსპორტირებულია",
    changePassword: "პაროლის შეცვლა",
    subtitle: "თქვენი პროდუქტიულობის ცენტრი",
    profile: " პროფილი",
    category: "კატეგორია",
    details: "დეტალები",
    additionalInfo: "Დამატებითი ინფორმაცია",
    taskStatistics: "დავალების სტატისტიკა",
    eventStatistics: "ღონისძიების სტატისტიკა",
    financialSummary: "Ფინანსური შეჯამება",
    summaryStatistics: "შეჯამების სტატისტიკა",
    eventsData: "მოვლენების მონაცემები",
  },
  events: {
    submitBookingRequest: "დაჯავშნის მოთხოვნის გაგზავნა",
    fullNameRequired: "Სრული სახელ��",
    fullName: "Სრული სახელი",
    phoneNumber: "Ტელეფონის ნომერი",
    paymentStatus: "Გადახდის სტატუსი",
    selectPaymentStatus: "აირჩიეთ გადახდის სტატუსი",
    paymentAmount: "გადახდის თანხა",
    startDateTime: "Დაწყების თარიღი და დრო",
    endDateTime: "Დასრულების თარიღი და დრო",
    eventNotes: "შენიშვნები",
    addEventNotes: "დაამატეთ შენიშვნები თქვენი დაჯავშნის მოთხოვნის შესახებ",
    editEvent: "ღონისძიების რედაქტირება",
    addNewEvent: "ახალი ღონისძიების დამატება",
    updateEvent: "ღონისძიების განახლება",
    createEvent: "ღონისძიების შექმნა",
    socialLinkEmail: "ელფოსტა / სოციალური ქსელის ბმული",
    dateAndTime: "თარიღი და დრო",
    date: "თარიღი",
    time: "დრო",
    start: "Დაწყება",
    end: "Დასრულება",
    eventDetails: "ღონისძიების დეტალები",
    title: "ღონისძიების სათაური"
  },
  contact: {
    email: "ელფოსტა",
    getInTouch: "Დაგვიკავშირდით",
    contactInfo: "საკონტაქტო ინფორმაცია",
    phone: "ტელეფონი",
    address: "მისამართი",
    addressLine1: "123 ბიზნესის ქუჩა",
    addressLine2: "ქალაქი, ქვეყანა",
    businessHours: "Სამუშაო საათები",
    workingHours: "ორშაბათი-პარასკევი: 9:00-17:00",
    weekendHours: "შაბათ-კვირა: დაკეტილია",
    sendMessage: "მოგვწერეთ შეტყობინება",
    name: "სახელი",
    namePlaceholder: "შეიყვანეთ თქვენი სახელი",
    emailPlaceholder: "შეიყვანეთ თქვენი ელფოსტა",
    message: "შეტყობინება",
    messagePlaceholder: "დაწერეთ თქვენი შეტყობინება აქ",
    send: "გაგზავნა",
    sending: "გაგზავნა...",
    messageSent: "შეტყობინება გაგზავნილია",
    messageSentDesc: "გმადლობთ, თქვენი შეტყობინება მიღებულია",
    error: "შეცდომა",
    errorDesc: "თქვენი შეტყობინების გაგზავნისას მოხდა შეცდომა. გთხოვთ, სცადოთ ხელახლა."
  },
  calendar: {
    attachment: "დანართი",
    day: "დღე",
    week: "კვირა",
    month: "თვე",
    addEvent: "ღონისძიების დამატება",
    bookNow: "Დაჯავშნეთ ახლა",
    weekOf: "კვირა"
  },
  common: {
    cancel: "გაუქმება",
    success: "Წარმატება",
    error: "შეცდომა",
    submitting: "გაგზავნა...",
    loading: "იტვირთება...",
    rateLimitReached: "მიღწეულია შეზღუდვა",
    waitBeforeBooking: "გთხოვთ, დაელოდოთ {{time}} სანამ სხვა დაჯავშნის მ���თხოვნას გაგზავნით",
    rateLimitMessage: "თქვენ შეგიძლიათ გააგზავნოთ მხოლოდ ერთი დაჯავშნის მოთხოვნა ყოველ 2 წუთში",
    waitTimeRemaining: "გთხოვთ დაელოდოთ",
    of: "დან",
    attachments: "დანართები",
    refreshing: "მონაცემების განახლება...",
    copiedToClipboard: "დაკოპირებულია ბუფერში",
    copyError: "ტექსტის კოპირების შეცდომა",
    deleteSuccess: "წარმატებით წაიშალა",
    deleteError: "წაშლის შეცდომა",
    missingUserInfo: "მომხმარებლის ინფორმაცია აკლია",
    errorOccurred: "მოხდა შეცდომა. გთხოვთ, სცადოთ ხელახლა.",
    new: "ახალი",
    request: "მოთხოვნა",
    requests: "მოთხოვნები",
    backToHome: "Სახლში დაბრუნება"
  },
  auth: {
    welcome: "კეთილი იყოს თქვენი მობრძანება SmartBookly-ში",
    description: "მართეთ თქვენი ბიზნესი ჩვენი სრული გადაწყვეტილებით",
    signInButton: "Შესვლა",
    signUpButton: "Რეგისტრაცია",
    emailLabel: "ელფოსტა",
    passwordLabel: "პაროლი",
    usernameLabel: "მომხმარებლის სახელი",
    forgotPassword: "პაროლი დაგავიწყდათ?",
    noAccount: "ანგარიში არ გაქვთ?",
    haveAccount: "უკვე გაქვთ ანგარიში?",
    signInCta: "Შესვლა",
    signUpCta: "Რეგისტრაცია",
    agreeToTerms: "ვეთანხმები წესებსა და პირობებს",
    requiredField: "ეს ველი სავალდებულოა",
    invalidEmail: "გთხოვთ, შეიყვანოთ სწორი ელფოსტა",
    passwordMinLength: "პაროლი უნდა შედგებოდეს მინიმუმ 6 სიმბოლოსგან",
    passwordMatch: "პაროლები არ ემთხვევა",
    confirmPassword: "პაროლის დადასტურება",
    resetPassword: "პაროლის აღდგენა",
    resetPasswordCta: "აღდგენის ბმულის გაგზავნა",
    resetPasswordSuccess: "პაროლის აღდგენის ელფოსტა გაგზავნილია",
    resetPasswordError: "აღდგენის ელფოსტის გაგზავნის შეცდომა",
    signInError: "არასწორი ელფოსტა ან პაროლი",
    signUpError: "ანგარიშის შექმნის შეცდომა",
    businessNameLabel: "Ბიზნესის სახელი",
    businessTypeLabel: "Ბიზნესის ტიპი",
    selectBusinessType: "აირჩიეთ ბიზნესის ტიპი",
    fullNameLabel: "Სრული სახელი",
    fullName: "Სრული სახელი",
    phoneLabel: "ტელეფონი",
    terms: "პირობები",
    privacy: "კონფიდენციალურობა",
    and: "და",
    resetInstructions: "შეიყვანეთ თქვენი ელფოსტა და ჩვენ გამოგიგზავნით ინსტრ���ქციას, თუ როგორ აღადგინოთ თქვენი პაროლი.",
    signingIn: "შესვლა...",
    signingUp: "რეგისტრაცია...",
    loading: "იტვირთება...",
    passwordRequirements: "პაროლი უნდა შედგებოდეს მინიმუმ 6 სიმბოლოსგან",
    confirmPasswordLabel: "პაროლის დადასტურება",
    enterEmail: "შეიყვანეთ თქვენი ელფოსტა",
    redeemCode: "კოდის გამოყენება",
    redeemCodeOptional: "კოდის გამოყენება (სურვილისამებრ)",
    enterRedeemCode: "შეიყვანეთ თქვენი კოდი",
    sendResetLink: "აღდგენის ბმულის გაგზავნა",
    backToSignIn: "უკან შესვლა"
  },
  legal: {
    title: "Ვადები და კონფიდენციალურობის პოლიტიკა",
    lastUpdated: "ბოლო განახლება: {{date}}",
    termsOfService: "მომსახურების პირობები",
    welcomeText: "კეთილი იყოს თქვენი მობრძანება SmartBookly-ში. ეს მომსახურების პირობები ('პირობები') არეგულირებს თქვენს მიერ ჩვენი SaaS პლატფორმისა და სერვისების ('სერვისები') გამოყენებას. ჩვენს სერვისებზე წვდომით ან გამოყენებით, თქვენ ეთანხმებით ამ პირობებს. თუ არ ეთანხმებით, გთხოვთ, არ გამოიყენოთ ჩვენი სერვისები.",
    generalInfo: {
      title: "1. Ზოგადი ინფორმაცია",
      companyName: "კომპანიის სახელი: SmartBookly",
      registered: "რეგისტრირებულია: საქართველო",
      contact: "საკონტაქტო ელფოსტა: info@smartbookly.com"
    },
    eligibility: {
      title: "2. Უნარი",
      description: "თქვენ უნდა იყოთ მინიმუმ 18 წლის ჩვენი სერვისების გამოსაყენებლად. ჩვენი პლატფორმის გამოყენებით, თქვენ ადასტურებთ, რომ აკმაყოფილებთ ამ მოთხოვნას."
    },
    accountSecurity: {
      title: "3. Ანგარიშის რეგისტრაცია და უსაფრთხოება",
      requirements: [
        "თქვენ უნდა მიაწოდოთ ზუსტი და სრული ინფორმაცია ანგარიშის შექმნისას.",
        "თქვენ ხართ პასუხისმგებელი თქვენი ანგარიშისა და პაროლის კონფიდენციალურობის შენარჩუნებაზე.",
        "დაუყოვნებლივ შეგვატყობინეთ თქვენს ანგარიშზე ნებისმიერი არაავტორიზებული წვდომის შესახებ."
      ]
    }
  }
};
