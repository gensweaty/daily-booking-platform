import { TranslationType } from "./types";

export const ka: TranslationType = {
  nav: {
    signin: "Შესვლა",
    startJourney: "მოგზაურობის დაწყება",
    contact: "კონტაქტი",
  },
  hero: {
    title: "გაზარდეთ თქვენი ბიზნესის პროდუქტიულობა SmartBookly-ით",
    subtitle: "ყოვლისმომცველი გადაწყვეტა - ღონისძიებების დაჯავშნა, ამოცანების მართვა, CRM და ანალიტიკა",
    description: "სრულად აკონტროლეთ თქვენი სამუშაო პროცესი ჩვენი უპრობლემო დაჯავშნის კალენდრით, ამოცანების მძლავრი მენეჯმენტით, მომხმარებლებთან ურთიერთობის ჭკვიანური მენეჯმენტით და ინტეგრირებული ანალიტიკით. ყველაფერი რაც თქვენს ბიზნესს სჭირდება, ერთ სივრცეში.",
  },
  features: {
    title: "რას გთავაზობთ",
    mainTitle: "ყველაფერი რაც თქვენს ბიზნესს სჭირდება წარმატებისთვის",
    booking: "ჯავშნები",
    tasks: "ამოცანები",
    crm: "CRM",
    analytics: "ანალიტიკა",
    website: "ვებ-გვერდი",
    ownBookingWebsite: "თქვენი საკუთარი დაჯავშნის ვებ-გვერდი",
    businessTitle: "ნებისმიერი ტიპის ბიზნესისთვის",
    smartAppointment: "შეხვედრების ჭკვიანური დაგეგმვა",
    automatedAnalytics: "ავტომატიზირებული შესრულების ანალიზი",
    modernCRM: "თანამედროვე CRM გადაწყვეტა",
    kanbanManagement: "ამოცანების მართვა Kanban-ით"
  },
  booking: {
    title: "ჯავშნების სისტემა",
    description: "მართეთ შეხვედრები და ღონისძიებები ეფექტურად",
    feature1: "კალენდრის მრავალი ხედი (თვე, კვირა, დღე)",
    feature2: "ღონისძიებების დაგეგმვა მორგებული დროის მონაკვეთებით",
    feature3: "კლიენტების ჯავშნების მართვა გადახდების თვალყურის დევნებით",
    feature4: "ღონისძიებების ავტომატური სინქრონიზაცია CRM-თან",
    feature5: "ჯავშნის შეტყობინებები ელფოსტით და დაფიდან",
    requestSubmitted: "თქვენი ჯავშნის მოთხოვნა წარმატებით გაიგზავნა!",
    yourEmailPlaceholder: "შეიყვანეთ თქვენი ელფოსტა",
    bookAppointment: "ვიზიტის დაჯავშნა"
  },
  analytics: {
    title: "ინტეგრირებული ანალიტიკა",
    description: "თვალყური ადევნეთ თქ���ენს შესრულებას და ზრდას",
    feature1: "ჯავშნებისა და შემოსავლების ანალიზი",
    feature2: "მორგებული ფილტრაცია თარიღების დიაპაზონის მიხედვით",
    feature3: "შემოსავლების შედარება თვეებს შორის",
    feature4: "ვიზუალური ინტერაქტიული მეტრიკები და გრაფიკები",
    feature5: "Excel-ში ჩამოტვირთვა ერთი დაწკაპუნებით"
  },
  crm: {
    title: "ურთიერთობების მართვა კლიენტებთან",
    description: "ააწყვეთ და შეინარჩუნეთ მყარი ურთიერთობები კლიენტებთან ჩვენი CRM-ის საშუალებით",
    feature1: "კლიენტების ინფორმაციის ცენტრალიზებული მართვა",
    feature2: "ფაილების მიმაგრება და დოკუმენტების ორგანიზება",
    feature3: "გადახდების თვალყურის დევნება და სტატუსის მონიტორინგი",
    feature4: "ელასტიური ძიება მონაცემებზე სწრაფი წვდომისთვის",
    feature5: "Excel-ში ჩამოტვირთვა ერთი დაწკაპუნებით ყველა ნაჩვენები მონაცემისთვის",
    notPaid: "გადაუხდელი",
    paidPartly: "ნაწილობრივ გადახდილი",
    paidFully: "სრულად გადახდილი",
    newCustomer: "ახალი კლიენტი",
    fullNameRequired: "Სრული სახელი",
    fullNamePlaceholder: "შეიყვანეთ კლიენტის სრული სახელი",
    phoneNumber: "Ტელეფონის ნომერი",
    phoneNumberPlaceholder: "შეიყვანეთ ტელეფონის ნომერი",
    socialLinkEmail: "ელფოსტა / სოციალური ქსელის ბმული",
    socialLinkEmailPlaceholder: "შეიყვანეთ ელფოსტა ან სოციალური ქსელის ბმული",
    createEventForCustomer: "ღონისძიების შექმნა ამ კლიენტისთვის",
    customerFromEvent: "ეს კლიენტი შეიქმნა ღონისძიებიდან",
    paymentStatus: "გადახდის სტატუსი",
    paymentAmount: "გადახდის თანხა",
    selectPaymentStatus: "აირჩიეთ გადახდის სტატუსი",
    comment: "კომენტარი",
    commentPlaceholder: "დაამატეთ კომენტარი ამ კლიენტის შესახებ",
    create: "Შექმნა",
    cancel: "გაუქმება",
    addCustomer: "კლიენტის დამატება",
    editCustomer: "კლიენტის რედაქტირება",
    update: "განახლება",
    dates: "თარიღები",
    actions: "მოქმედებები",
    customersPerPage: "კლიენტები თითო გვერდზე",
    noCustomers: "კლიენტები ვერ მოიძებნა",
    noCustomersDescription: "დაამატეთ თქვენი პირველი კლიენტი დასაწყებად",
    refreshing: "მონაცემების განახლება...",
    yes: "კი",
    no: "არა",
    copiedToClipboard: "კოპირებულია ბუფერში",
    copyError: "ტექსტის კოპირების შეცდომა",
    deleteSuccess: "წარმატებით წაიშალა",
    deleteError: "წაშლის შეცდომა",
    missingUserInfo: "მომხმარებლის ინფორმაცია აკლია",
    customerCreated: "კლიენტი წარმატებით შეიქმნა",
    customerUpdated: "კლიენტი წარმატებით განახლდა",
    noDataToExport: "ექსპორტისთვის მონაცემები არ არის",
    error: "შეცდომა"
  },
  tasks: {
    title: "ამოცანების მართვა",
    description: "იყავით ორგანიზებული და პროდუქტიული",
    feature1: "Kanban დაფა ამოცანების ვიზუალური ორგანიზებისთვის",
    feature2: "სტატუსის თვალყურის დევნება და პროგრესის მონიტორინგი",
    feature3: "ამოცანების ეფექტური პრიორიტიზაცია",
    feature4: "მარტივი გადატანისა და ჩამოგდების ფუნქციონირება",
    feature5: "ამოცანებისთვის შენიშვნების სწრაფი შენახვა",
    addTask: "ამოცანის დამატება",
    editTask: "ამოცანის რედაქტირება",
    titlePlaceholder: "ამოცანის სათაური",
    descriptionLabel: "აღწერა",
    todo: "გასაკეთებელი",
    inProgress: "მიმდინარე",
    done: "დასრულებული",
    deleteTaskConfirmTitle: "ამოცანის წაშლა",
    taskUpdated: "ამოცანა წარმატებით განახლდა",
    taskDeleted: "ამოცანა წარმატებით წაიშალა"
  },
  website: {
    title: "უფასო ვებ-გვერდი თქვენი ბიზნესისთვის",
    description:
      "მიიღეთ პროფესიონალური დაჯავშნის ვებ-გვერდი თქვენი ბიზნესისთვის ყოველგვარი კოდის გარეშე.",
    feature1: "პროფესიონალური დიზაინი",
    feature2: "მობილურზე ადაპტირებული",
    feature3: "მომსახურების ჩამონათვალი",
    feature4: "ონლაინ დაჯავშნის ინტეგრაცია",
    feature5: "შეხვედრების დამტკიცება ელექტრონული ფოსტით",
  },
  cta: {
    title: "მზად ხართ გაზარდოთ თქვენი ბიზნესი?",
    subtitle: "დაიწყეთ ჩვენი პლატფორმის გამოყენება დღეს და ნახეთ განსხვავება!",
    button: "Დაიწყე ახლა",
  },
  footer: {
    copyright: "© 2023 თქვენი კომპანიის სახელი. Ყველა უფლება დაცულია.",
    terms: "მომსახ���რების პირობები",
    privacy: "კონფიდენციალურობის პოლიტიკა",
    description: "ბიზნესის მართვის პლატფორმა სერვისის როვაიდერებისთვის",
    navigation: "ნავიგაცია",
    legal: "იურიდიული",
    termsAndPrivacy: "პირობები და კონფიდენციალურობა",
    rights: "Ყველა უფლება დაცულია",
  },
  business: {
    events: "ღონისძიებები და გართობა",
    health: "ჯანმრთელობა და კეთილდღეობა",
    sports: "სპორტი და ფიტნესი",
    beauty: "სილამაზე და სპა",
    personal: "პირადი მომსახურება",
    education: "განათლება და ქოუჩინგი",
    eventsDesc: "ღონისძიებების დამგეგმავები, არტისტები, ლოკაციები და სხვა",
    healthDesc: "ექიმები, თერაპევტები, კეთილდღეობის მწვრთნელები",
    sportsDesc: "პირადი მწვრთნელები, სპორტული მწვრთნელები, ფიტნეს ცენტრები",
    beautyDesc: "პარიკმახერები, ბარბერები, ფრჩხილების სალონები, სპა ცენტრები",
    personalDesc: "კონსულტანტები, ფოტოგრაფები, საოჯახო მომსახურება",
    educationDesc: "მასწავლებლები, რეპეტიტორები, ცხოვრების მწვრთნელები, ვორქშოფების პროვაიდერები",
    myBusiness: "Ჩემი ბიზნესი",
    businessProfile: "Ბიზნეს პროფილი",
    bookingRequests: "ჯავშნის მოთხოვნები",
    pendingRequests: "მოთხოვნები მოლოდინში",
    approvedRequests: "დადასტურებული მოთხოვნები",
    rejectedRequests: "უარყოფილი მოთხოვნები",
    noPendingRequests: "არ არის მოთხოვნები მოლოდინში",
    noApprovedRequests: "არ არის დადასტურებული მოთხოვნები",
    noRejectedRequests: "არ არის უარყოფილი მოთხოვნები",
    pendingRequestsDescription: "როდესაც მომხმარებლები გამოაგზავნიან ჯავშნის მოთხოვნებს, ისინი აქ გამოჩნდებიან თქვენი დასამტკიცებლად.",
    approvedRequestsDescription: "დადასტურებული ჯავშნის მოთხოვნები აქ გამოჩნდება.",
    rejectedRequestsDescription: "უარყოფილი ჯავშნის მოთხოვნები აქ გამოჩნდება.",
    editBusinessProfile: "Ბიზნეს პროფილის რედაქტირება",
    chooseFile: "აირჩიეთ ფაილი",
    noFileChosen: "ფაილი არ არის არჩეული",
    supportedFormats: "მხარდაჭერილი ფორმატები: .jpg, .jpeg, .png, .pdf, .docx, .xls",
    uploadImageCover: "ატვირთეთ სურათი თქვენი ��იზნესის ყდისთვის (JPEG, PNG, WebP)",
    publicPageUrl: "ეს გამოყენებული იქნება თქვენი საჯარო გვერდის URL-ისთვის:",
    notFound: "ბიზნსი ვერ მოიძებნა",
    notFoundDescription: "ბოდიშს გიხდით, ჩვენ ვერ ვიპოვეთ ბიზნესი ამ URL-ით. გთხოვთ, გადაამოწმოთ URL და სცადოთ ხელახლა.",
    viewPublicPage: "საჯარო გვერდის ნახვა",
    customer: "მომხმარებელი",
    title: "სათაური",
    dateTime: "თარიღი და დრო",
    actions: "მოქმედებები",
    approve: "დადასტურება",
    reject: "უარყოფა",
    delete: "წაშლა",
    deleteBookingRequest: "ჯავშნის მოთხოვნის წაშლა",
    deleteConfirmation: "დარწმუნებული ხართ, რომ გსურთ ამ ჯავშნის მოთხოვნის წაშლა? ამ მოქმედების გაუქმება შეუძლებელია.",
    businessName: "Ბიზნესის სახელი",
    urlSlug: "URL სლოგი",
    description: "აღწერა",
    coverPhoto: "ყდის ფოტო",
    phone: "ტელეფონი",
    email: "Ელექტრონული ფოსტა",
    website: "Ვებ გვერდი",
    address: "მისამართი",
    updateProfile: "პროფილის განახლება",
    availableTimes: "ხელმისაწვდომი დროები",
    clickToRequest: "დააწკაპუნეთ ნებისმიერ დროს ჯავშნის მოთხოვნის გასაგზავნად",
    contactInformation: "საკონტაქტო ინფორმაცია"
  },
  dashboard: {
    welcome: "კვლავ მოგესალმებით",
    greeting: "გამარჯობა",
    goodMorning: "Დილა მშვიდობისა",
    goodAfternoon: "Შუადღე მშვიდობისა",
    goodEvening: "Საღამო მშვიდობისა",
    goodNight: "Ღამე მშვიდობისა",
    overview: "Მიმოხილვა",
    calendar: "Კალენდარი",
    bookingCalendar: "ჯავშნების კალენდარი",
    tasks: "Დავალებები",
    reminders: "შეხსენებები",
    notes: "Შენიშვნები",
    business: "Ბიზნესი",
    stats: "Სტატისტიკა",
    statistics: "Სტატისტიკა",
    crm: "CRM",
    signOut: "Გამოსვლა",
    darkMode: "Მუქი რეჟიმი",
    lightMode: "Ნათელი რეჟიმი",
    systemMode: "Სისტემა",
    more: "მეტი",
    addEvent: "ღონისძიების დამატება",
    day: "Დღე",
    week: "Კვირა",
    month: "Თვე",
    totalTasks: "სულ ამოცანები",
    completed: "დასრულებული",
    inProgress: "პროცესში",
    todo: "გასაკეთებელი",
    totalEvents: "სულ ღონისძიებები",
    partlyPaid: "ნაწილობრივ გადახდილი",
    fullyPaid: "სრულად გადახდილი",
    totalIncome: "სულ შემოსავალი",
    fromAllEvents: "ყველა ღონისძიებიდან",
    exportSuccessful: "ექსპორტი წარმატებულია",
    exportSuccessMessage: "თქვენი მონაცემები წარმატებით ექსპორტირებულია",
    changePassword: "აროლის შეცვლა",
    subtitle: "თქვენი პროდუქტიულობის ცენტრი",
    profile: "პროფილი",
    category: "კატეგორია",
    details: "დეტალები",
    additionalInfo: "Დამატებითი ინფორმაცია",
    taskStatistics: "დავალებების სტატისტიკა",
    eventStatistics: "ღონისძიებების სტატისტიკა",
    financialSummary: "Ფინანსური შეჯამება",
    summaryStatistics: "შეჯამების სტატისტიკა",
    eventsData: "მონაცემები მოვლენების შესახებ",
  },
  events: {
    submitBookingRequest: "ჯავშნის მოთხოვნის გაგზავნა",
    fullNameRequired: "Სრული სახელი",
    fullName: "Სრული სახელი",
    phoneNumber: "Ტელეფონის ნომერი",
    paymentStatus: "გადახდის სტატუსი",
    selectPaymentStatus: "აირჩიეთ გადახდის სტატუსი",
    paymentAmount: "გადახდის თანხა",
    startDateTime: "Დაწყების თარიღი და დრო",
    endDateTime: "Დასრულების თარიღი და დრო",
    eventNotes: "Შენიშვნები",
    addEventNotes: "დაამატეთ შენიშვნები თქვენი ჯავშნის მოთხოვნის შესახებ",
    editEvent: "ღონისძიების რედაქტირება",
    addNewEvent: "ახალი ღონისძიების დამატება",
    updateEvent: "ღონისძიების განახლება",
    createEvent: "ღონისძიების შექმნა",
    deleteEvent: "ღონისძიების წაშლა",
    eventDeleted: "ღონისძიება წარმატებით წაიშალა",
    eventUpdated: "ღონისძიება წარმატებით განახლდა",
    eventCreated: "ივენთი წარმატებით შეიქმნა",
    timeSlotUnavailable: "დროის მონაკვეთი მიუწვდომელია",
    timeSlotConflict: "დროის კონფლიქტი",
    timeSlotNotAvailable: "ეს დროის მონაკვეთი მიუწვდომელია",
    socialLinkEmail: "ელფოსტა / სოციალური ქსელის ბმული",
    dateAndTime: "თარიღი და დრო",
    date: "თარიღი",
    time: "Დრო",
    start: "Დაწყება",
    end: "Დასრულება",
    eventDetails: "ღონისძიების დეტალები",
    title: "ღონისძიების სათაური",
    bookAppointment: "შეხვედრის დაჯავშნა",
    submitRequest: "მოთხოვნის გაგზავნა"
  },
  contact: {
    email: "ელექტრონული ფოსტა",
    getInTouch: "Დაგვიკავშირდით",
    contactInfo: "საკონტაქტო ინფორმაცია",
    phone: "Ტელეფონი",
    address: "მისამართი",
    addressLine1: "123 ბიზნესის ქუჩა",
    addressLine2: "ქალაქი, ქვეყანა",
    businessHours: "Სამუშაო საათები",
    workingHours: "ორშაბათი-პარასკევი: 9:00-17:00",
    weekendHours: "შაბათი-კვირა: დაკეტილია",
    sendMessage: "გამოგვიგზავნეთ შეტყობინება",
    name: "სახელი",
    namePlaceholder: "შეიყვანეთ თქვენი სახელი",
    emailPlaceholder: "შეიყვანეთ თქვენი ელექტრონული ფოსტა",
    message: "შეტყობინება",
    messagePlaceholder: "დაწერეთ თქვენი შეტყობინება აქ",
    send: "Გაგზავნა",
    sending: "იგზავნება...",
    messageSent: "შეტყობინება გაგზავნილია",
    messageSentDesc: "გმადლობთ, თქვენი შეტყობინება მიღებულია",
    error: "შეცდომა",
    errorDesc: "თქვენი შეტყობინების გაგზავნისას მოხდა შეცდომა. გთხოვთ, სცადოთ ხელახლა."
  },
  calendar: {
    attachment: "დანართი",
    day: "Დღე",
    week: "Კვირა",
    month: "Თვე",
    addEvent: "ივენთის დამატება",
    bookNow: "დაჯავშნე ვიზიტი",
    weekOf: "კვირა"
  },
  common: {
    cancel: "გაუქმება",
    success: "შესრულდა",
    error: "შეცდომა",
    warning: "გაფრთხილება",
    submitting: "იგზავნება...",
    loading: "იტვირთება...",
    rateLimitReached: "სიხშირის ლიმიტი მიღწეულია",
    waitBeforeBooking: "გთხოვთ, დაელოდოთ {{time}} სანამ სხვა ჯავშნის მოთხოვნას გამოაგზავნით",
    rateLimitMessage: "თქვენ შეგიძლიათ გამოაგზავნოთ ჯავშნის მოთხოვნა ყოველ 2 წუთში ერთხელ",
    waitTimeRemaining: "გთხოვთ, დაელოდოთ",
    of: "დან",
    attachments: "დანართები",
    refreshing: "მონაცემების განახლება...",
    copiedToClipboard: "კოპირებულია ბუფერში",
    copyError: "ტექსტის კოპირების შეცდომა",
    deleteSuccess: "წარმატებით წაიშალა",
    deleteError: "წაშლის შეცდომა",
    missingUserInfo: "მომხმარებლის ინფორმაცია აკლია",
    errorOccurred: "მოხდა შეცდომა. გთხოვთ, სცადოთ ხელახლა.",
    new: "ახალი",
    request: "მოთხოვნა",
    requests: "მოთხოვნები",
    backToHome: "მთავარ გვერდზე დაბრუნება",
    create: "Შექმნა",
    update: "განახლება",
    delete: "წაშლა",
    and: "და",
    supportedFormats: "მხარდაჭერილი ფორმატები: .jpg, .jpeg, .png, .pdf, .docx, .xls",
    deleteConfirmMessage: "დარწმუნებული ხართ, რომ გსურთ წაშლა? ეს ქმედება შეუქცევადია.",
    noDescription: "აღწერა არ არის",
    deleteConfirmTitle: "გსურთ წაშლა?"
  },
  auth: {
    welcome: "მოგესალმებით SmartBookly-ში",
    description: "მართეთ თქვენი ბიზნეს ჩვენი ყოვლისმომცველი გადაწყვეტით",
    signInButton: "Შესვლა",
    signUpButton: "Რეგისტრაცია",
    emailLabel: "Ელექტრონული ფოსტა",
    passwordLabel: "პაროლი",
    usernameLabel: "მომხმარებლის სახელი",
    forgotPassword: "დაგავიწყდათ პაროლი?",
    noAccount: "არ გაქვთ ანგარიში?",
    haveAccount: "უკვე გაქვთ ანგარიში?",
    signInCta: "Შესვლა",
    signUpCta: "Რეგისტრაცია",
    agreeToTerms: "ვეთანხმები წესებსა და პირობებს",
    requiredField: "ეს ველი სავალდებულოა",
    invalidEmail: "გთხოვთ, შეიყვანოთ სწორი ელექტრონული ფოსტა",
    passwordMinLength: "პაროლი უნდა შეიცავდეს მინიმუმ 6 სიმბოლოს",
    passwordMatch: "პაროლები არ ემთხვევა",
    confirmPassword: "პაროლის დადასტურება",
    resetPassword: "პაროლის აღდგენა",
    resetPasswordCta: "აღდგენის ბმულის გაგზავნა",
    resetPasswordSuccess: "პაროლის აღდგენის ელფოსტა გაგზავნილია",
    resetPasswordError: "აღდგენის ელფოსტის გაგზავნის შეცდომა",
    signInError: "არასწორი ელექტრონული ფოსტა ან პაროლი",
    signUpError: "ანგარიშის შექმნის შეცდომა",
    businessNameLabel: "Ბიზნესის სახელი",
    businessTypeLabel: "Ბიზნესის ტიპი",
    selectBusinessType: "აირჩიეთ ბიზნესის ტიპი",
    fullNameLabel: "Სრული სახელი",
    fullName: "Სრული სახელი",
    phoneLabel: "Ტელეფონი",
    terms: "პირობები",
    privacy: "კონფიდენციალურობა",
    and: "და",
    resetInstructions: "შეიყვანეთ თქვენი ელ.ფოსტა და ჩვენ გამოგიგზავნით ინსტრუქციას, თუ როგორ აღადგინოთ თქვენი პაროლი.",
    passwordRequirements: "პაროლი უნდა შეიცავდეს მინიმუმ 6 სიმბოლოს",
    confirmPasswordLabel: "პაროლის დადასტურება",
    signingIn: "შესვლა...",
    signingUp: "რეგისტრაცია...",
    loading: "იტვირთება...",
    enterEmail: "შეიყვანეთ თქვენი ელ.ფოსტა",
    redeemCode: "გამოსყიდვის კოდი",
    redeemCodeOptional: "გამოსყიდვის კოდი (არასავალდებულო)",
    enterRedeemCode: "შეიყვანეთ თქვენი გამოსყიდვის კოდი",
    sendResetLink: "გადატვირთვის ბმულის გაგზავნა",
    backToSignIn: "უკან შესვლა",
    passwordsDoNotMatch: "პაროლები არ ემთხვევა",
    passwordTooShort: "პაროლი ძალიან მოკლეა",
    verificationRequired: "მნიშვნელოვანი: რეგისტრაციის შემდეგ, თქვენ უნდა დააჭიროთ ვერიფიკაციის ბმულს, რომელიც გამოგზავნილია თქვენს ელ.ფოსტაზე, რათა გააქტიუროთ თქვენი ანგარიში.",
    checkEmailFolders: "გთხოვთ, შეამოწმოთ როგორც შემომავალი, ასევე სპამის საქაღალდეები დადასტურების ელ.ფოსტისთვის."
  },
  bookings: {
    approvedBooking: "დადასტურებული ჯავშანი"
  },
  legal: {
    title: "Წესები და კონფიდენციალურობის პოლიტიკა",
    lastUpdated: "ბოლო განახლება: {{date}}",
    termsOfService: "მოგესალმებით SmartBookly-ში. ეს მომსახურების პირობები ('პირობები') არეგულირებს ჩვენი SaaS პლატფორმისა და სერვისების ('სერვისები') გამოყენებას. ჩვენს სერვისებზე წვდომით ან გამოყენები, თქვენ ეთანხმებით ამ პირობებს. თუ არ ეთანხმებით, გთხოვთ, არ გამოიყენოთ ჩვენი სერვისები.",
    welcomeText: "მოგესალმებით SmartBookly-ში. ეს მომსახურების პირობები ('პირობები') არეგულირებს ჩვენი SaaS პლატფორმისა და სერვისების ('სერვისები') გამოყენებას. ჩვენს სერვისებზე წვდომით ან გამოყენები, თქვენ ეთანხმებით ამ პირობებს. თუ არ ეთანხმებით, გთხოვთ, არ გამოიყენოთ ჩვენი სერვისები.",
    generalInfo: {
      title: "1. Ზოგადი ინფორმაცია",
      companyName: "კომპანიის სახელი: SmartBookly",
      registered: "რეგისტრირებულია: საქართველო",
      contact: "საკონტაქტო ელფოსტა: info@smartbookly.com"
    },
    eligibility: {
      title: "2. Უნარიანობა",
      description: "თქვენ უნდა იყოთ მინიმუმ 18 წლის ჩვენი სერვისების გამოსაყენებლად. ჩვენი პლატფორმის გამოყენებით, თქვენ ადასტურებთ, რომ აკმაყოფილებთ ამ მოთხოვნას."
    },
    accountSecurity: {
      title: "3. Ანგარიშის რეგისტრაცია და უსაფრთხოება",
      requirements: [
        "ანგარიშის შექმნისას უნდა მიუთითოთ ზუსტი და სრული ინფორმაცია.",
        "თქვენ ხართ პასუხისმგებელი თქვენი ანგარიშისა და პაროლის კონფიდენციალურობის შენარჩუნებაზე.",
        "დაუყოვნებლივ შეგვატყობინეთ თქვენს ანგარიშზე ნებისმიერი არაავტორიზებული ��ვდომის შესახებ."
      ]
    }
  }
};
