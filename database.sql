...
BEGIN
  ...
  RAISE NOTICE '🔍 Event data received: is_recurring=%, repeat_pattern=%, repeat_until=%, start_date=%, reminder_enabled=%, reminder_at=%', 
               v_is_recurring, v_repeat_pattern, v_repeat_until, v_start_date::date, v_email_reminder_enabled, v_reminder_at;
  ...
  IF p_event_id IS NULL THEN
    ...
    INSERT INTO events (
      ...
      reminder_at, email_reminder_enabled, created_at, updated_at
    ) VALUES (
      v_safe_title,
      v_safe_user_surname,
    ...
    RAISE NOTICE '📝 Created parent event: % with recurring: %, pattern: %, until: %, reminder: %', 
                 v_event_id, v_is_recurring, v_repeat_pattern, v_repeat_until, v_email_reminder_enabled;
